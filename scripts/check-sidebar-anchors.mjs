...
for (const { text, link } of leaves) {
  ...
  if (pageName === "") pageName = "index"; // home page link "/" or "/#anchor"
  const filePath = join(docsDir, `${pageName}.md`);
  if (!existsSync(filePath)) {
    errors.push(
      `sidebar link '${link}' -> file not found: docs/${pageName}.md`,
    );
    continue;
  }
  checked++;
...
for (const [pageName, anchors] of pageLeaves) {
  ...
  if (pageName === "index") continue; // home sidebar is a brief anchor nav, not a full section index
  const filePath = join(docsDir, `${pageName}.md`);
  const headings = collectHeadings(readFileSync(filePath, "utf8"));
  for (const h of headings) {
...
console.log(
  `(sidebar parsed via ${source}: ${leaves.length} leaf items, ${groups.length} groups)`,
);
...
console.log(
  `PASS: sidebar anchors OK (${checked} link(s) checked forward, ${pageLeaves.size} page(s) checked reverse)`,
);
process.exit(0);
