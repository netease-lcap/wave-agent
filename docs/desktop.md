...
- **设置页**：输入 `/config` 打开设置页，切换到"项目设置"选项卡，打开「SDD」开关（写入项目 .wave/settings.json；桌面端会先弹重启确认框，由你选「立即重启」或「稍后重启」）。
...
插件市场在「设置 → 插件市场」中打开（在对话中输入 `/plugin` 也会直接跳到该视图）。按市场 Tab 浏览插件，可用「全部 / 已安装 / 未安装」筛选或按关键词搜索，点击行内「安装」后选择安装作用域（用户 / 项目 / 本地），一键安装；安装后即可在对话中使用。

安装后插件从市场克隆到缓存目录（`~/.wave/plugins/cache/`）。

![插件市场](/screenshots/desktop-plugin-market.webp)
...
已安装插件在插件市场的「已安装」筛选中可见，行内展示当前安装作用域（点击「用户 ▾」可更换作用域或卸载）。

![已安装插件](/screenshots/desktop-plugin-installed.webp)

#### 3）更新插件

插件市场清单在打开插件市场界面时自动刷新（**所有市场，不区分来源，也没有开关**）——进入「设置 → 插件市场」即触发，刷新期间工具栏显示「检查更新中…」；刷新只拉取市场检出内容，**不会**自动升级已安装的插件，宿主启动与对话初始化都不刷新。插件升级有两个显式入口：插件行内出现新版本时的「更新」（单个插件），或插件市场工具栏的「批量更新插件」（批量升级该市场的插件，按进入视图时刷新好的清单执行、自身不再拉取检出，仅已安装插件中版本真的变化的才会提示「已更新 N 个插件」）。有新版本时版本号显示为「已安装 vX · 最新 vY」。

#### 4）添加新市场

插件市场工具栏「＋ 新建市场」支持两种来源：**本地路径**（点击「选择文件夹」选取本地插件目录）与**远程仓库**（填 GitHub `owner/repo` 或完整 Git 地址）。市场名称由市场自身的 `marketplace.json` 清单决定，无需手填；同一来源已添加过、或其清单名称与已有市场同名时不允许添加，并提示原因。
...
设置页是 CodeWave IDE 桌面端的配置管理中心，从侧边栏底部账户卡片的菜单中的「设置」打开（覆盖会话区的全页面，侧边栏保持可见）。左侧导航按三组共 8 项组织：「通用」（全局设置 / 个性化）、「工作区」（项目设置）、「AI 与扩展」（插件市场 / 技能 / 子代理 / 钩子 / MCP 服务），当前激活项有高亮态。
...
内置插件 SDD（规格驱动开发）开关——该视图的唯一交互控件：切换写回项目 `.wave/settings.json` 的 `enabledPlugins`（自动创建或更新功能规格说明）。插件变更（安装 / 卸载 / 启用 / 禁用 / 更新，含本开关）需要重建对话才能生效：保存后桌面端弹确认框「插件变更需要重启对话才能生效。」，并提示「将重启 N 个对话，其中 M 个正在执行任务暂不重启。」，由你选择「立即重启」或「稍后重启」（Esc 等同「稍后重启」）——选「立即重启」也只重建空闲会话，正在执行任务的会话保持原配置。详见 [4.1 SDD](#_4-1-sdd)。

![项目设置](/screenshots/desktop-settings-project.webp)

插件变更后的重启确认框（无「取消」——变更此时已落盘，弹窗只决定生效时机）：

![插件变更重启确认框](/screenshots/desktop-settings-rebuild-prompt.webp)

### 6.5 插件市场、技能、子代理、钩子与 MCP 服务

「AI 与扩展」组下的五个视图，插件市场用于浏览与安装，其余按来源分组浏览当前生效的配置：

- **插件市场**：按市场 Tab 浏览插件（带插件数），支持 全部/已安装/未安装 筛选与关键词搜索；行内安装（选作用域）/ 更新 / 更换作用域与卸载；市场切换右侧可 批量更新插件 / 移除市场，右侧「＋ 新建市场」为全局入口。打开该视图时会在后台刷新各市场清单（工具栏显示「检查更新中…」）。详见 [5.2 插件](#_5-2-插件)
