...
设置页是 CodeWave IDE 桌面端的配置管理中心，从侧边栏底部账户卡片「更多」菜单中的「设置」打开（覆盖会话区的全页面，侧边栏保持可见）。左侧导航按三组共 7 项组织：「通用」（全局设置 / 个性化）、「工作区」（项目设置）、「AI 与扩展」（技能 / 子代理 / 钩子 / MCP 服务），当前激活项有高亮态。

### 6.1 账户卡片与设置入口

登录后，侧边栏底部显示当前账户卡片：用量概要常驻在卡片上半部分（套餐进度条 + API 双值金额，可经个人信息行右侧 chevron 收起/展开，显隐不受菜单影响），下方为头像与姓名。**点击头像/姓名热区弹出纯功能菜单**（宽度与卡片等宽、内容不溢出）——仅包含设置 / 企业控制台 / 帮助文档 / 退出登录（与 IDE 插件的「更多」菜单一致），再次点击个人信息或失焦/Esc 收起；用量信息由常驻区单独承载，不进菜单。个人信息行右侧在有新版本时显示「更新」按钮（下载二次确认 → 下载中禁用 → 就绪自动弹重启确认），不在热区内——点更新只触发更新流程，不打开菜单：

![账户卡片](/screenshots/desktop-account-card.webp)

### 6.2 全局设置

「基础设置」卡片展示当前生效的系统语言与上下文长度（K 值）：

- **系统语言**：下拉选择，中文（zh-CN）/ English（en-US）
...
新版本发布时，桌面端在界面内展示更新提示，可点击执行更新操作。

![更新提示](/screenshots/desktop-update-toast.webp)

![更新操作](/screenshots/desktop-update-toast-action.webp)
