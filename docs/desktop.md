...
- **二级**：每个目录下最近 5 条会话，按创建时间倒序排序（最新创建在上，后续活跃不改变位置）
...
点击目标元素后，旁边弹出评论卡片，写下要改什么，回车或点击发送图标提交；× 或点击卡片外空白处可取消并重新选择：

![元素评论](/screenshots/desktop-preview-comment.png)

提交的评论会作为一条用户消息发送到当前会话，附带页面 URL、元素选择器和元素摘要，agent 据此精确理解「指的是哪里」并修改原型。提交后自动退出拾取模式（一次评论一个元素）；页面导航或刷新后拾取自动重置为关闭。
