...
插件市场清单在打开插件市场界面时自动刷新（**所有市场，不区分来源，也没有开关**）——进入「设置 → 插件市场」即触发，刷新期间工具栏显示「检查更新中…」；刷新只拉取市场检出内容，**不会**自动升级已安装的插件，宿主启动与对话初始化都不刷新。插件升级有两个显式入口：插件行内出现新版本时的「更新」（单个插件），或插件市场工具栏的「批量更新插件」（批量升级该市场的插件，仅已安装插件中版本真的变化的才会提示「已更新 N 个插件」）。有新版本时版本号显示为「已安装 vX · 最新 vY」。
