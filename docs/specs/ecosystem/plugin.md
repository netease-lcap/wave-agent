...
- **FR-029**：IDE 插件必须提供 `/plugin` 斜杠命令打开插件管理对话框。
- **FR-030**：IDE 插件管理对话框必须提供"探索"、"已安装"、"市场"三个标签页，与 CLI 界面的三个导航区域（FR-013）对应。
- **FR-031**："探索"页必须支持按插件名称和描述实时搜索过滤，并在点击插件后显示详情视图，详情视图必须提供项目作用域（默认）、用户作用域、仅此仓库三个安装按钮（作用域语义见 FR-008）。
- **FR-032**："已安装"页必须仅列出已安装插件（含作用域、来源市场、版本），并为每个插件提供"更新"和"卸载"操作。
- **FR-033**："市场"页必须列出已注册市场的名称、来源地址和上次更新时间，提供添加市场输入框，并为每个市场提供"更新"和"移除"操作。
- **FR-034**：插件安装或卸载完成后，IDE 插件必须使变更在当前会话中即刻生效（重建会话能力，如插件命令、技能的注册或移除）。
- **FR-035**：IDE 插件管理对话框中的操作失败时，必须通过 IDE 原生错误提示展示失败原因，而非对话框内联错误。
- **FR-036**：IDE 插件管理对话框必须支持通过点击对话框外区域、按下 Escape 或点击关闭按钮关闭。
