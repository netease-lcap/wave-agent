...
作为用户，我希望 MCP 服务器通过 `initialize` 自述的使用说明（`instructions`）在它变为可用的那一刻**播报一次**，这样"限流是多少、调用前要满足什么前提、整个服务器怎么用"这类**不调用也要知道**的内容不会丢失，同时服务器在会话中途连接时也不会让已经缓存的前缀失效。
