...
2. **假设** MCP 服务器已连接，且该服务器的工具不参与工具延迟加载（见 [`core/tool-deferred-loading.md`](../core/tool-deferred-loading.md)：延迟加载未生效，或该工具被逐工具标注为始终扁平声明），**当** 代理列出工具时，**则** 它必须包含带有 `mcp__` 前缀的 MCP 工具（逐条声明）。
3. **假设** AI 决定调用 MCP 工具，**当** 工具执行时，**则** 请求必须发送到正确的 MCP 服务器并将结果返回给 AI——无论这个工具是以逐条声明还是经工具延迟加载（`ToolInvoke`，`namespace` 即该服务器的服务器名）转发的方式暴露给 AI。
4. **假设** 工具延迟加载生效（默认可 defer 的工具数 ≥ 阈值时，见 [`core/tool-deferred-loading.md`](../core/tool-deferred-loading.md)），**当** 代理列出工具时，**则** 这些 MCP 工具不作为逐条独立工具出现，其能力改由延迟加载（`ToolInvoke` + 紧凑目录，条目记法 `<namespace>.<tool>(签名)`，此时 `namespace` = 服务器名）暴露。
5. **假设** 某 MCP 工具被逐工具标注为始终扁平声明（opt-out，等价 Claude Code 的 `alwaysLoad`），**当** 代理列出工具时，**则** 该工具必须始终逐条声明、不得进入延迟加载，同一服务器其余未标注的工具仍按默认策略暴露。
6. **假设** 某 MCP 服务器暴露大量工具（如数十个），**当** 其目录被渲染时，**则** 每个条目必须紧凑（工具名 + 参数摘要，过长描述须被裁剪），目录整体规模不得随工具描述长度无界增长。
...
- **工具名称冲突**：来自不同服务器的同名工具通过添加 `mcp__[serverName]__[toolName]` 前缀来处理；该全名是工具对外的唯一标识（权限规则、审批提示、持久化规则与 `--tools` 白名单均使用它）。当工具经延迟加载暴露时，目录条目与 `ToolInvoke` 使用 `<namespace>.<tool>` 寻址（MCP 工具的 `namespace` 即其服务器名），两字段**分列**（消歧依赖模型显式给出两者，而不是把一个可能含分隔符的长名再拆开），仅在转发调用落到叶子时组合为 `mcp__[serverName]__[toolName]` 全名。
- **不支持的 Schema 字段**：包含 `$schema`、`exclusiveMinimum` 或 `exclusiveMaximum` 等字段的 MCP 工具 schema 必须被清理以确保与 LLM API 的兼容性；经延迟加载暴露时，目录条目中的参数摘要必须基于清理后的 schema 渲染（清理规则对逐条声明与目录摘要一视同仁）。
- **无效配置**：如果 `.mcp.json` 格式错误，代理应记录错误并在没有 MCP 工具的情况下继续。
- **配置合并**：当存在多个配置源（构造函数、项目 `.mcp.json`、用户 `~/.wave/mcp.json`、插件）时，它们按以下优先级合并：构造函数 > 项目（.mcp.json）> 用户（~/.wave/mcp.json）> 插件服务器；工具的「始终扁平声明」标注是**逐工具**的，随所属服务器的配置一并按上述优先级合并（同名服务器被覆盖时，其工具标注也随覆盖者）。
- **始终扁平声明与延迟加载的互斥**：某工具被标注为始终扁平声明后，该工具不得同时出现在延迟加载的目录中（避免同一能力有两种声明形态，造成调用路径与审批对象二义）。
