...
### 用户故事：把服务器自带的使用说明交给模型（优先级：P2）

作为用户，我希望 MCP 服务器通过 `initialize` 自述的使用说明（`instructions`）能进入系统提示，这样"限流是多少、调用前要满足什么前提、整个服务器怎么用"这类**不调用也要知道**的内容不会丢失。

**优先级原因**：这是上下文保真度而非工具能力。工具描述是可以被目录压缩的（模型要看原文得先搜索命中，见 `docs/specs/core/exec-tool.md`），而服务器级的使用约束一旦被压缩掉，模型就不知道自己在违反什么限制。

**独立测试**：连接一个在 `initialize` 响应里返回多行 `instructions` 的 MCP 服务器，断言系统提示中出现该段原文并标明来源服务器；断开后断言其消失。

**验收场景**：

1. **假设** 某 MCP 服务器在 `initialize` 响应里返回了 `instructions`，**当** 连接成功时，**则** `McpManager` 必须保存这段文本，不得丢弃。
2. **假设** 至少一个已连接的服务器带有 `instructions`，**当** 组装系统提示时，**则** 必须把这些说明注入系统提示，并标明每段来自哪台服务器（多台服务器时彼此可区分）。
3. **假设** `instructions` 是多行文本，**当** 注入时，**则** 必须原样保留（含换行），不得按宽度裁剪、也不得只保留首行。
4. **假设** 某服务器的工具被权限规则全部排除，**当** 组装系统提示时，**则** 不得注入该服务器的 `instructions`（说明与工具同源，不能让被拒服务器的内容绕过规则进入上下文）；服务器一个工具都没有时照常注入。
5. **假设** 服务器断开、连接失败或转为 `error`，**当** 后续组装系统提示时，**则** 必须清掉该服务器上一次的 `instructions`，不得让已下线服务器的说明继续留在上下文里。
6. **假设** 所有已连接服务器都没有 `instructions`，**当** 组装系统提示时，**则** 不得产生空段落或只有标题的空章节（无内容 = 不输出）。
7. **假设** 工具池已收敛、MCP 工具不再逐条扁平声明，**当** 组装系统提示时，**则** `instructions` 仍必须出现——它是服务器级上下文，不随工具声明形态变化。
...
- **服务器级 `instructions` 与工具描述的职责边界？** 服务器级的 `instructions` 走系统提示、不参与目录预算也不裁剪，适合承载"不调用也要知道"的内容（限流、前置条件、整体用法约定）；单个工具的描述会被目录压缩、要拿原文得靠搜索命中，所以不要把服务器级约束写进工具描述里。
- **`instructions` 进系统提示会不会让前缀缓存失效？** 会——服务器连接/断开或 `instructions` 变化都会改变系统提示文本；这与"工具池变化本来就会改变工具声明"属同一类变化。按 `docs/specs/core/prompt-cache-control.md` 的分块约定，它属于动态内容（`cacheable: false`），不得进入静态块；但说明文本自身不得含时间戳、连接状态等动态内容，池不变时文本逐字节相同。
