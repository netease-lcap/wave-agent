...
### 用户故事：按来源 Tab 展示钩子与项目分组（优先级：P1）

作为 GUI 用户，我希望设置页「钩子」选项卡按来源以 Tab 形式展示钩子（用户级 / 项目级 / 插件），其中项目级再按所属项目分组、每个项目一张卡片，以便快速定位某个来源或某个项目下的钩子，并知道它属于哪一类（用户 / 项目 / 插件）。

**为什么是这个优先级**：钩子来源决定其适用范围（用户级全局生效 / 项目级仅当前项目），Tab 化 + 项目分组让长列表可扫读，与技能页形态统一（2026-08-29 用户拍板：钩子与技能页一致，按来源 Tab + 项目分组卡片展示；钩子创建成功后默认开启，用户可选择关闭）。

**独立测试**：在具备用户级、项目级、插件钩子的环境中打开设置页钩子选项卡，验证来源 Tab 存在、项目级按项目分组为卡片、每张卡片列出该项目下的钩子并可执行管理操作。

**验收场景**：

1. **假设** 环境中存在用户级、项目级、插件来源的钩子，**当** 钩子选项卡打开，**则** 顶部展示来源 Tab（用户级钩子 / 项目级钩子 / 插件钩子），点击切换显示对应来源的钩子
2. **假设** 存在项目级钩子，**当** 用户位于「项目级钩子」Tab，**则** 钩子按所属项目分组展示，每个项目一张卡片，卡片头显示项目名
3. **假设** 某项目卡片内存在多个钩子，**当** 卡片展示，**则** 列出该项目下的所有钩子（事件、描述、命令），并提供「编辑」「删除」操作入口
...
2. **假设** 用户在「项目级钩子」Tab 某项目卡片内点击「新增」，**当** 输入框预填，**则** 预填项目级提示词并带上该项目名，如 `帮我在【项目名称】新建钩子：<事件>时，<matcher 工具>，跑<命令>，<超时/异步>`
...
5. **假设** 新建的是项目级钩子，**当** 钩子页「项目级钩子」Tab 展示，**则** 新钩子出现在所属项目的卡片列表中
...
- **项目级钩子按项目分组的项目归属如何确定？** 从配置文件路径推断（settings.json 位于哪个工作目录即属于哪个项目）；路径无法推断时归入「其他项目」分组
