...
>
> - **`title`（可选，仅 `.html` 生效）**：artifact 的标题（浏览器标签 / 画廊显示名）。服务端解析阶梯 = 页面**前 8KB 内**的 `<title>` > `title` 参数 > 默认名——标签**永不**被参数覆盖（客户端不自行判优先级，也不做冲突提示）；客户端按 CC 的末级兜底补**文件名 basename**，因此 `.html` 发布**总会发一个非空 `title`**，标题链才闭合。
