...
2. **假设**信息已保存在 `MEMORY.md` 中，**当**新会话开始时，**则**`MEMORY.md` 的前 200 行必须包含在系统提示中；**当**索引同时超过 200 行或超过 25000 字节时，**则**必须按两个上限一并截断（先按行、再按字节且在换行处切分），并附加一条写明触发原因（行数 / 字节）与「单行索引应短于约 200 字符，细节移到主题文件」的警告
...
1. **假设**主代理通过 Read/Grep/Glob 触及某文件，**当**该文件所在目录或其某个祖先目录存在 `AGENTS.md`（不存在时回退 `CLAUDE.md`）且该文件尚未被注入过，**则**必须把其内容作为 `<system-reminder>Contents of <path>:` 注入，并在处理完本次触达集合后清空待注入集合
2. **假设**同一嵌套记忆文件已被注入过，**当**后续再次触达该目录下的文件时，**则**不得重复注入
3. **假设**该文件同时命中路径特定记忆规则（`.wave/rules/` 的 `paths`），**则**两者都必须独立生效，嵌套注入不得取代路径规则注入
4. **假设**嵌套目录链上存在多个 `AGENTS.md`，**则**必须按「距文件由近到远」的顺序注入，或至少以最近的那个为准（不得只注入根目录的那个）
...
- **大型记忆文件**：如果记忆文件增长过大，系统应该高效处理。对于 `MEMORY.md`，只加载前 200 行且不超过 25000 字节（行数上限挡不住超长单行，实测出现过 200 行内接近 200 KB 的索引），超出时截断并给出原因与「单行索引应短于约 200 字符」的警告
