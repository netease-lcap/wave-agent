...
作为 CLI 用户，我希望压缩（手动 `/compact` 或自动触发）进行期间，在 "✻ Compacting message history..." 加载提示后看到当前流式传输文本的最后 30 个字符，以便我知道压缩摘要正在生成并能实时看到输出进度。
...
1. **假设**压缩进行中（手动或自动触发）且模型尚无任何流式输出，**则** CLI 在消息列表下方显示 "✻ Compacting message history..."，不显示流式尾部。
2. **假设**压缩进行中且模型已输出文本（普通文本或推理内容，推理经内容通道混合），**则** CLI 在 "✻ Compacting message history..." 后显示该文本的最后 30 个字符；文本不超过 30 字符时原样显示，超过 30 字符时以 `…` 前缀截断（换行折叠为 `\n`，与主对话流式尾部样式一致）。
