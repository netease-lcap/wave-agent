...
- **边界情况 9**：工具声明（`tools[]`）与工具描述位于缓存前缀内，因此必须逐字节稳定——同一配置下不得混入预算、时间、连接状态等易变信息。**`Exec` 折叠生效时，工具池的变化不再改写 `tools[]` 的任何一个字节**：它的工具描述是与池无关的**常量**（不含任何 `mcp__` 名、目录正文与截断行），目录改在池变化时作为一条 `isMeta` 消息追加到消息尾部（见 `docs/specs/core/exec-tool.md`），**只追加、不改写已有历史**。未被折叠的扁平 MCP 工具声明仍随服务器连接/断开而变化——那确实改变了前缀内容，属于允许的变更。
