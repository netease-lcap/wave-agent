...
- **边界情况 9**：工具声明（`tools[]`）与工具描述位于缓存前缀内，因此必须逐字节稳定——同一工具池、同一配置下，工具描述（含 `Exec` 的目录渲染，见 `docs/specs/core/exec-tool.md`）不得混入预算、时间、连接状态等易变信息。工具池本身发生真实变化（MCP 服务器连接/断开导致收敛阈值跨越）时允许变更，因为这确实改变了前缀内容。
