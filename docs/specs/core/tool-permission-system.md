...
4. **假设**模型经工具延迟加载（见 [`core/tool-deferred-loading.md`](./tool-deferred-loading.md)）的 `ToolInvoke` 调用某 MCP 叶子且没有匹配规则，**当**权限检查发生时，**则**确认提示必须按**叶子工具全名**（`mcp__server__tool`）标识这次调用，而非 `ToolInvoke` 的名字。
5. **假设**显示了经 `ToolInvoke` 发起的叶子调用确认提示，**当**用户选择"是，不再询问"时，**则**持久化规则必须是叶子全名 `mcp__server__tool`（与逐条声明时写入的规则完全同形），不得把 `ToolInvoke` 的名字写入规则。
6. **假设**存在针对叶子全名 `mcp__server__tool` 的允许规则，**当**该叶子经 `ToolInvoke` 被调用时，**则**它必须立即执行而不提示（规则命中不因外层是 `ToolInvoke` 而失效）。
7. **假设**权限模式为 `dontAsk` 且某叶子未被预批准，**当**该叶子经 `ToolInvoke` 被调用时，**则**它必须被拒绝（`ToolInvoke` 本身不构成对该叶子的放行）。
8. **假设**存在针对 `ToolInvoke` 的拒绝规则，**当**模型尝试任意转发调用时，**则**该调用必须被整体拒绝；反之，针对 `ToolInvoke` 的允许规则**不得**被当作任意叶子的放行依据。
9. **假设**模型经 `ToolInvoke({ namespace: "builtin", tool: "Bash", args: { command: "npm test" } })` 调用内置叶子，**当**评估带参数的模式规则（如 `Bash(npm test)`）时，**则**必须使用**叶子自己的名字与叶子自己的参数**（`args.command`）匹配，不得拿 `namespace` / `tool` 字段或 `ToolInvoke` 的入参结构去匹配；内置叶子经转发调用时，其既有的确认流程与「不再询问」持久化（如 `Bash(npm test)`）照旧成立。
...
- **叶子**：最终被执行的工具，也是权限判定的单位。MCP 叶子 = `mcp__server__tool`；内置叶子 = 工具名本身（如 `Read` / `Bash` / `Edit`），带参数的模式如 `Bash(npm test)` 用**叶子自己的参数**匹配。无论工具是逐条声明给模型、还是经工具延迟加载的 `ToolInvoke` 被调用，权限规则、审批提示与持久化规则的单位都必须是叶子（`ToolInvoke` 的名字与它的 `namespace` / `tool` 字段均不参与叶子的身份与规则匹配——`namespace` 只是寻址用的命名空间）。
...
- **受限与不受限工具**：不受限工具（不在 `RESTRICTED_TOOLS` 中的工具）仍应被自动允许，即使在 `dontAsk` 模式下。经工具延迟加载的 `ToolInvoke` 调用不改变这一归属——`ToolInvoke` 与 `ToolSearch` 本身不是受限工具，受限的是它所调用的叶子（`mcp__` 前缀工具，以及本身受限的内置工具如 `Edit` / `Write` / `Bash`），因此叶子的审批/拒绝语义必须照常生效，不得因外层调用是「自动允许的工具」而被跳过。
- **一条调用含两层身份时的规则归属**：这类调用的允许/拒绝判定必须以**叶子**为准（叶子名 + 叶子自己的参数）；对 `ToolInvoke` 的规则只能整体开/关转发能力（拒绝时整体阻断），不能替代叶子粒度的授权；`namespace` 字段只是寻址用的命名空间，不参与规则匹配。
