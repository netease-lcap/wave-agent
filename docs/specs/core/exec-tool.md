...
**独立测试**：用小预算渲染一个较大池，断言条目数受限、每台服务器都出现一行摘要（含"一个席位都没拿到"写成 `none shown` 的情形）、且末尾出现带 "X of Y" 与搜索路径的 `PARTIAL` 行。

**验收场景**：

1. **假设** `Exec` 被声明，**当** 渲染其描述时，**则** 目录必须给出每个工具的可达写法、参数签名与描述；签名是多行 TypeScript 形态（对象按字段逐行展开、可选字段带 `?`、数组写成 `Array<T>`），字段描述与约束以 JSDoc 形式挂在对应字段上方，且**必须保留 schema 里的原文**（多行照排、不按宽度裁剪），约束标签含 `@default`、`@minItems`/`@maxItems`、`@format`、`@deprecated`；**只有工具自身的描述**才取首行并截断到固定宽度；合法的标识符名用 `tools.<name>(...)`，非标识符名（含 `-`、`.` 等）必须用 `tools["<name>"](...)` 括号形式（否则会被解析成减法/属性链）。
...
7. **假设** 模型在目录里找不到想要的工具、也说不出它的名字，**当** 它读 `Exec` 的描述时，**则** 描述必须写明查询串为空会列出**完整池**（含与目录同形态的签名），使模型有一条不依赖任何关键词的枚举路径——这是"截断不藏存在性"在模型侧的兑现方式；该说明是静态文案，不得含预算等调优参数。
8. **假设** 模型按 `Exec` 描述里给出的形式调用 search，**当** 调用发生时，**则** 必须成功——描述里的调用形式与宿主接受的参数形状必须由**同一份 schema** 导出（签名由同一个渲染器渲染、入参按同一份 schema 校验），不得文案与实现各写一份；"描述教位置参数、实现只收对象"这类漂移正是这条要根除的缺陷。
...
- **搜索入口的单一来源**：沙箱内搜索的**调用形式与签名**必须由同一份 schema 渲染（复用与目录同形的签名渲染器），宿主也按这份 schema 校验入参；描述里不得手写调用形式——文案与实现各写一份正是二者漂移的根因。
...
- **目录预算够用吗？** 默认预算对齐 opencode：按常见 MCP 工具的描述密度可完整容纳数十个工具，超过即显式截断（按服务器轮转保留，不按池顺序一刀切）。截断是预期行为（宁可显式截断 + 可搜索，也不要静默超预算），搜索覆盖完整池。截断态下另为每台服务器输出一行不计预算的摘要（对齐 opencode 的「namespace 计数行」）——轮转只保证"预算允许时每台先各得一席"，极端块尺寸下某台仍可能一个席位都拿不到；摘要行让它在那种情况下至少被点到名。
...
- **search 传了位置参数（非对象）怎么办？** 仍由沙箱的载荷契约拒绝（"参数必须是普通对象"）——沙箱只接受普通对象是既有的隔离属性，不为搜索开口子。因为描述里的形式由 schema 渲染，模型不会从文档里学到这种写法。
- **MCP 服务器写在使用说明里的整段散文放哪？** 那是服务器级内容（`initialize` 的 `instructions`），走独立的播报通道（服务器可用时追加一条消息，不进系统提示）、不参与目录预算也不裁剪，见 `docs/specs/ecosystem/mcp.md`；不要把限流、注意事项这类"不调用也要知道"的内容写进单个工具的字段描述里——那是最容易被压缩掉的位置。
