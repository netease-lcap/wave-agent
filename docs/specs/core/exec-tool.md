...
- **MCP 服务器写在使用说明里的整段散文放哪？** 那是服务器级内容（`initialize` 的 `instructions`），走系统提示通道、不参与目录预算也不裁剪，见 `docs/specs/ecosystem/mcp.md`；不要把限流、注意事项这类"不调用也要知道"的内容写进单个工具的字段描述里——那是最容易被压缩掉的位置。
