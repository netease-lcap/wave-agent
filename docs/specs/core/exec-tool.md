...
作为用户，我希望对话流里 Exec 调用的折叠行显示脚本的首个非空行，以便不展开就能看出这次编排要做什么。

**为什么是这个优先级**：折叠行是 Exec 调用在消息流中的常显信息（结果行只报调用次数），而脚本是多行代码——整段贴上去会撑开折叠行，取首行则是 Claude Code 里同类沙箱工具的做法。

**独立测试**：让 agent 用多行脚本调用 Exec，验证折叠行只显示首个非空行，且只在超长时才出现省略号。

**验收场景**：

1. **假设** `code` 含多行（含前后空行），**当** 生成紧凑文本时，**则** 只取首个非空行（去掉该行首尾空白），输出中不出现任何换行符，也不把后续行拼进来。
2. **假设** 首个非空行超过 50 个字符，**当** 生成紧凑文本时，**则** 截断为 49 个字符并追加 `…`（总长 50 个字符，省略号占 1 位）。
3. **假设** 首个非空行不超过 50 个字符，**当** 生成紧凑文本时，**则** 原样显示（除首尾空白外不做改动），不追加省略号。
4. **假设** `code` 缺失或为纯空白，**当** 生成紧凑文本时，**则** 紧凑文本为空串——只显示工具名，不追加参数文本，也不出现 `undefined`/`null`。
...
- **紧凑文本是纯展示层的？** 是。它不进模型上下文、不参与脚本校验与执行，也不影响权限判定；取不到脚本内容时只是降级为空串（折叠行只剩工具名）。
- **截断为什么按字符数而不是显示宽度？** 与 Claude Code 的 REPL 同口径（它取首行后用 `.length` 比较）。按显示宽度截断需要 SDK 侧新增宽度工具（本仓库只有 `packages/code` 用了 `string-width`），收益是常数级，本功能不做——含义是 CJK 行按字符计，长中文行显示上会略短于等字符数的英文行。
- **内层调用在对话流里怎么显示？** 目前只在 Exec 的结果行里以次数出现（`Exec · N tool calls`），不为每次嵌套调用单独出行；把内层调用物化成独立行（Claude Code 的 REPL 那样）是另一项展示改动，不在本规格范围内。
