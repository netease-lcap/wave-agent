...
4. **假设** 脚本试图通过 `tools.someTool.constructor("return process")()` 之类的路径回到能编译代码的函数，**当** 脚本执行时，**则** 必须同样抛 `EvalError`——沙箱里所有函数对象（包括每个工具包装函数与 `console` 方法）都必须属于沙箱 realm，不得把宿主 realm 的函数直接交出去。
...
- **可观测**：沙箱崩溃、超时、abort 都记日志，并且都以失败工具结果的形式回到模型（不抛异常、不静默）。
...
- **搜索返回的内容会不会很大？** 搜索返回完整池的匹配条目（含原始 schema）时可能较大；它与其它工具调用一样计入调用上限，且工具结果本身受既有截断机制约束。
