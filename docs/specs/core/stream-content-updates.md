...
### 用户故事：思考块展开/收起平滑过渡与最短展示时长（优先级：P2）

用户在查看助手的推理（思考）块时，希望思考结束时收起动作平滑、不因思考过程很短而出现"展开又立刻收起"的闪烁，以便阅读不被打断。

**为什么是这个优先级**：思考块在流式开始时展开、结束时自动收起（见「思考用时展示」故事），但收起是瞬时的高度跳变；思考过程很短（如不足 1 秒）时表现为内容一闪而过，观感突兀。平滑过渡与最短展示时长是低成本高感知的观感打磨，故 P2。

**独立测试**：渲染一个 `stage` 从 `"streaming"` 变为 `"end"` 的推理块，验证收起高度以约 200ms 过渡动画渐变而非瞬间消失；构造思考时长不足 1 秒的块，验证其保持展开至满 1 秒后才开始收起；构造思考时长超过 1 秒的块，验证结束时立即开始平滑收起。

**验收场景**：

1. **假设**推理块内容区正在展开或收起，**当**高度变化发生时，**则**高度以约 200ms 的过渡动画平滑变化，而非瞬间跳变
2. **假设**推理块生成结束（`stage` 变为 `"end"`），**当**其从开始展示到结束的时长不足 1 秒时，**则**保持展开状态直至展示满 1 秒后才自动收起（避免"快速展开快速收起"的闪烁）
3. **假设**推理块生成结束，**当**其展示时长已达 1 秒以上时，**则**立即开始自动收起（不因最短展示时长而额外延迟）
4. **假设**用户点击思考块头部手动展开或收起，**当**点击发生时，**则**手动操作立即生效且同样以平滑过渡呈现；若手动收起发生在待自动收起期间，则取消自动收起的等待，不与之冲突
5. **假设**历史会话加载的已结束推理块（挂载时即 `stage="end"`），**当**渲染时，**则**默认收起且不播放展开过渡动画（避免历史回看时内容区闪动）

---

### 用户故事：Bash 命令输出平滑展开（优先级：P2）

作为 IDE/桌面端用户，我希望 Bash 工具执行时，输出首次到达后内容区以平滑过渡展开而不是瞬间跳高，以便视野不被突兀的高度变化打断。

**为什么是这个优先级**：Bash 工具块在命令刚显示时只有命令一行，输出到达后结构从"仅命令"整体切换为"命令 + 输出"，且运行中输出持续增长，造成瞬时高度跳变（观感晃动）。输出区展开平滑化与思考块收起平滑化（见上一故事）同属低成本高感知的观感打磨，故 P2。

**独立测试**：渲染一个 Bash 工具块（`stage="running"`）仅含命令、无 result，验证输出区不占高度；随后更新该块携带 result，验证输出区以约 200ms 过渡动画展开而非瞬间出现；渲染挂载时即携带 result 的块（如历史会话），验证直接展开、不播放过渡动画。

**验收场景**：

1. **假设** Bash 工具块仅含命令（result 为空），**当**渲染时，**则**只显示命令区域，输出区不占高度、无空行
2. **假设** Bash 工具块运行中首次获得 result，**当**输出内容出现时，**则**输出区以约 200ms 的过渡动画平滑展开，而非瞬间跳高
3. **假设**输出内容超过最大高度（如 120px），**当**渲染时，**则**输出区内部滚动，不撑高外部布局
4. **假设**历史会话加载的 Bash 工具块挂载时即携带完整 result，**当**渲染时，**则**输出区直接展开、不播放过渡动画（避免历史回看时内容区闪动）

---

### 用户故事：Write/Edit 工具内容平滑展开（优先级：P2）

作为 IDE/桌面端用户，我希望 Write 预览与 Edit 差异在工具完成时平滑展开而不是瞬间跳高，以便视野不被突兀的高度变化打断。

**为什么是这个优先级**：Write/Edit 工具在流式阶段仅显示工具标题，完成后一次性挂载内容预览/差异视图，造成瞬时高度跳变。与 Bash 输出平滑展开（见上一故事）同属低成本高感知的观感打磨，故 P2。

**独立测试**：渲染一个 Write/Edit 工具块，验证流式阶段仅显示标题、无内容区；随后将阶段更新为 `end`，验证内容区以约 200ms 动画平滑展开而非瞬间出现。

**验收场景**：

1. **假设** Write 工具处于流式阶段，**当**渲染时，**则**仅显示工具标题，不渲染内容预览
2. **假设** Write 工具完成（`stage="end"`），**当**内容预览挂载时，**则**预览区以约 200ms 动画平滑展开，而非瞬间跳高
3. **假设** Edit 工具完成（`stage="end"`），**当**差异视图挂载时，**则**差异区以约 200ms 动画平滑展开，而非瞬间跳高
4. **假设** Write 预览内容超过最大高度（如 120px），**当**渲染时，**则**预览区内部滚动，不撑高外部布局
