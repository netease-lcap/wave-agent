...
作为 IDE 插件用户，我希望通过 `/config` 斜杠命令或头部设置按钮打开设置页并选中「全局设置」选项卡，查看当前生效的语言、上下文长度与直连参数等配置，以便确认插件的运行环境配置。

**为什么是这个优先级**：这是未登录用户使用 IDE 插件的主要配置入口，直接影响插件可用性。设置页（批次 2）已提供「全局设置」导航项，`/config` 复用设置页入口避免维护独立的配置对话框（2026-08-29 用户拍板：弹窗内容迁移到设置页选项卡，对齐 `/agents` → 子代理、`/skills` → 技能）；设置页为只读浏览视图（2026-08-31 用户拍板：不做表单编辑与保存，配置修改走 CLI 命令与配置文件）。

**独立测试**：在 IDE 中输入 `/config` 或点击头部设置按钮，验证打开设置页并选中「全局设置」选项卡，只读展示当前已保存的配置值（无编辑与保存控件）。
...
3. **假设** 设置页「全局设置」视图已打开，**当** 页面渲染时，**则** 只读展示当前生效的界面语言（中文 / English）与上下文长度，无编辑控件与保存按钮。
4. **假设** 用户切换到设置页「直连设置」视图，**当** 页面渲染时，**则** 只读展示当前生效的 API Key、Base URL、Agent Model、Fast Model 值，无编辑控件与保存按钮。
5. **假设** 设置页已打开，**当** 用户点击「返回」或切换其他选项卡，**则** 回到会话视图或切换到对应选项卡，设置页不产生任何消息、不写入任何配置。
...
- 设置页（只读视图）加载已保存配置失败时如何展示？（设置页内显示错误与重试入口，不展示编辑表单）
