...
- **FR-042**：IDE 插件必须提供 `/config` 斜杠命令和聊天头部设置按钮两个入口打开配置对话框。
- **FR-043**：IDE 插件配置对话框必须提供"全局"和"模型"两个标签页；"全局"页提供界面语言选择（中文 / English），"模型"页提供 API Key（密码式隐藏输入）、自定义请求头、API 地址、模型名称、快速模型名称输入项。
- **FR-044**：IDE 插件配置对话框保存的配置必须存储在 IDE 侧（VS Code 与 JetBrains 插件各自的原生存储），不经由 CLI 子进程，两类 IDE 中行为一致。
- **FR-045**：IDE 插件配置保存成功后必须立即对所有活跃会话生效，无需重启 IDE 或重开会话。
- **FR-046**：IDE 插件配置对话框打开时必须载入当前已保存的配置供用户编辑。
- **FR-047**：保存进行中，IDE 插件配置对话框必须禁用所有输入与按钮并显示"保存中..."，防止重复提交；保存成功后对话框必须自动关闭。
- **FR-048**：配置加载或保存失败时，IDE 插件配置对话框必须在对话框内显示错误原因。
- **FR-049**：IDE 插件配置对话框必须在未完成配置时显示引导文案"保存后，优先使用此配置，无需登录即可正常使用插件。"
