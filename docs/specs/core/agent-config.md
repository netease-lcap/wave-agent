...
**为什么是这个优先级**：这是未登录用户使用 IDE 插件的主要配置入口，直接影响插件可用性。设置页（批次 2）已提供「全局设置」导航项，`/config` 复用设置页入口避免维护独立的配置对话框（2026-08-29 用户拍板：弹窗内容迁移到设置页选项卡，对齐 `/agents` → 子代理、`/skills` → 技能）。2026-08-31 曾拍板设置页为只读浏览视图（配置修改走 CLI 命令与配置文件）；2026-09-01 原型走查用户恢复可编辑：系统语言（下拉）与上下文长度（数字输入）提供编辑控件与「保存」按钮（对齐旧配置弹窗交互），保存经 `updateConfiguration` RPC 写回配置并即时生效，失败显示错误提示。

**独立测试**：在 IDE 中输入 `/config` 或点击头部设置按钮，验证打开设置页并选中「全局设置」选项卡，可编辑系统语言与上下文长度并保存；保存后配置生效且展示值刷新，保存失败有错误提示。
...
3. **假设** 设置页「全局设置」视图已打开，**当** 页面渲染时，**则** 展示当前生效的界面语言（中文 / English，下拉选择）与上下文长度（K 值，数字输入 16–1000），并提供「保存」按钮；保存后经 `updateConfiguration` 写回配置，成功回发新配置刷新展示，失败显示错误提示。
...
- 设置页加载已保存配置失败时如何展示？（设置页内显示错误与重试入口；编辑保存失败时展示错误提示并保留用户输入）
