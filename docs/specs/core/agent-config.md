...
**独立测试**：在 IDE 中输入 `/config` 或点击头部设置按钮，验证打开设置页并选中「全局设置」选项卡，可编辑 AI 回复语言与上下文长度并保存；保存后配置生效且展示值刷新，保存失败有错误提示。
...
3. **假设** 设置页「全局设置」视图已打开，**当** 页面渲染时，**则** 展示当前生效的 AI 回复语言（中文 / English，下拉选择，仅决定 AI 回复用语，不改变界面语言）与上下文长度（K 值，数字输入 16–1000），并提供「保存」按钮；保存后经 `updateConfiguration` 写回配置，成功回发新配置刷新展示，失败显示错误提示。
