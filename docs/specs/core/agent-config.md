...
作为 IDE 插件用户，我希望通过 `/config` 斜杠命令或头部设置按钮打开设置页并选中「全局设置」选项卡，填写语言、上下文长度等配置，以便无需登录即可正常使用插件。

**为什么是这个优先级**：这是未登录用户使用 IDE 插件的主要配置入口，直接影响插件可用性。设置页（批次 2）已提供「全局设置」导航项，`/config` 复用设置页入口避免维护独立的配置对话框（2026-08-29 用户拍板：弹窗内容迁移到设置页选项卡，对齐 `/agents` → 子代理、`/skills` → 技能）。

**独立测试**：在 IDE 中输入 `/config` 或点击头部设置按钮，验证打开设置页并选中「全局设置」选项卡，载入当前已保存的配置；修改并保存后新配置生效。
...
3. **假设** 设置页「全局设置」视图已打开，**当** 页面渲染时，**则** 提供界面语言选择（中文 / English）与上下文长度设置，并载入当前已保存的配置。
4. **假设** 用户切换到设置页「直连设置」视图，**当** 页面渲染时，**则** 提供 API Key、Base URL、Agent Model、Fast Model 字段，并载入当前已保存的配置；保存后优先使用此配置，无需登录即可使用插件。
5. **假设** 用户点击保存，**当** 保存进行中时，**则** 保存按钮禁用并显示进行中状态。
6. **假设** 保存成功，**当** 响应返回时，**则** 新配置立即对所有会话生效。
7. **假设** 保存或加载失败，**当** 失败发生时，**则** 设置页内显示错误原因。
8. **假设** 设置页已打开，**当** 用户点击「返回」或切换其他选项卡，**则** 回到会话视图或切换到对应选项卡，设置页不产生任何消息，未保存的修改不生效。
...
- IDE 插件配置入口（设置页「全局设置」）只填写部分字段（如仅语言、无上下文长度）时如何生效？（未填写的字段保持原值或回退到默认值）
- IDE 插件配置入口加载已保存配置失败时如何展示？（设置页内显示错误，表单仍可填写重试）
