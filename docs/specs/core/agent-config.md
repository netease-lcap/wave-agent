...
作为 IDE 插件用户，我希望通过 `/config` 斜杠命令或头部设置按钮打开设置页并选中「全局设置」选项卡，查看当前生效的语言、上下文长度等配置，以便确认插件的运行环境配置。
...
4. **假设** 设置页已打开，**当** 用户点击「返回」或切换其他选项卡，**则** 回到会话视图或切换到对应选项卡，设置页不产生任何消息、不写入任何配置。
