...
- **上游返回「未声明工具」的调用会怎样（`type: "custom"` 形状）？**
  - _事实记录_：如果模型为一个**不在本轮工具声明中**的工具名发起调用，上游可能返回非 `function` 形状的 tool call——`type: "custom"`，名称与输入位于 `custom.name` / `custom.input`（内网网关对该场景可稳定复现）。wave 的解析只识别 `function` 形状的 tool call，因此这类调用既不执行、也不产生任何可见提示（流式是默认路径），只能靠 `finish_reason` 仍为 tool_calls 间接察觉。⇒ 任何依赖「模型可直接调用未声明工具」的设计在本链路不成立：能力必须经由**已声明**的工具暴露（例如工具延迟加载的转发工具，见 [`core/tool-deferred-loading.md`](./tool-deferred-loading.md)）。
