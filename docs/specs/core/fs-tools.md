...
- 系统必须提供 `Read` 工具，支持文本、图片（PNG、JPEG、GIF、WebP）和 Jupyter 笔记本。
- `Read` 工具必须支持通过 `offset` 和 `limit` 进行分页，强制执行令牌级验证（使用 CJK 感知的 `estimateTokens` 估算令牌数，不得超过 `maxTokens`，默认 25000，来自 `context.fileReadingLimits.maxTokens`；CJK 字符按 1 字符≈1 令牌计算，其他字符 JSON/JSONL/JSONC 按 2 字符/令牌、其余按 4 字符/令牌计算），并提供结构化元数据。
- `Read` 工具必须支持去重，通过检查文件修改时间并在未变化时返回最小响应。
- `Read` 工具必须强制执行资源限制（默认 256KB）用于文件读取。仅当调用方显式提供 `limit` 参数时跳过大小检查，仅提供 `offset` 不跳过。
- `Read` 工具必须通过扩展名不区分大小写地检测图片文件并将内容转换为 base64 编码。
- `Read` 工具必须使用 base64 数据和正确的 MIME 类型填充 `ToolResult.images` 数组。
- `Read` 工具必须对图片文件强制执行 20MB 的文件大小限制。
- 系统必须提供自动创建父目录的 `Write` 工具。
- `Write` 工具在覆盖**已存在**的文件前必须验证该文件在当前会话中已被读取，防止意外数据丢失；若未读取则拒绝并返回指示先读取文件的消息。新建文件无需预先读取。
- 系统必须提供用于精确字符串替换的 `Edit` 工具，并带有详细的不匹配分析。
- `Edit` 工具必须拒绝编辑在当前对话中尚未读取的文件，返回指示代理先读取文件的错误。
- `Edit` 工具必须在匹配 `old_string` 之前将 CRLF（`\r\n`）规范化为 LF（`\n`），这样模型可以在 `old_string` 中使用 LF 来处理 CRLF 文件。
- 当未找到 `old_string` 时，`Edit` 工具必须在错误消息中包含尝试的 `old_string`（截断为 200 个字符）。
- `Edit` 工具提示应引导模型使用明显唯一的最小 `old_string`（通常为 2-4 行相邻文本），减少在长文本上的复现错误。
- `Edit` 工具在执行前必须进行陈旧检测：比较文件当前修改时间与 `readFileState` 中记录的修改时间。仅当当前修改时间晚于记录值（文件变新）时判定为可能被修改；此时若该次读取为完整读取（未提供 `offset` 和 `limit`）且文件当前内容哈希与记录值一致，则视为内容未变、允许继续；否则拒绝并返回"文件自上次读取后已被修改，请重新读取"的错误。分页读取（提供了 `offset` 或 `limit`）不享受内容兜底，mtime 变新即拒绝。
- `Write` 工具对已存在文件执行与 `Edit` 相同语义的陈旧检测（见上文 `Edit` 陈旧检测条目）：仅当文件变新（当前 mtime 晚于 `readFileState` 记录值）时判定为可能被修改；此时若为完整读取（未提供 `offset` 和 `limit`）且当前内容哈希与记录值一致，则视为内容未变、允许继续；否则拒绝并返回"文件自上次读取后已被修改，请重新读取"的错误。新建文件不进行陈旧检测。
- 系统必须提供用于快速模式匹配的 `Glob` 工具。它必须不遵循 `.gitignore`（但必须始终忽略 `.git` 目录），支持可配置的 `limit`，并提供结构化元数据。
- 系统必须提供基于 ripgrep 的 `Grep` 工具用于强大的文本搜索。它必须遵循 `.gitignore` 和常见忽略模式，支持 `offset` 和 `context` 参数，并提供结构化元数据。
- 所有工具必须与 `PermissionManager` 集成以进行授权。
- 系统必须提供带有词级高亮的可视化差异显示，用于逐行更改。
- 如果 `Grep` 工具未找到匹配项，输出必须包含建议指定 `path` 字段的提示。这是因为默认搜索路径是当前工作目录并遵循 `.gitignore`，指定路径（例如 `node_modules`）可以允许在忽略或特定目录中搜索。
