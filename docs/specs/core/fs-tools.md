...
- **FR-004**：`Write` 工具在覆盖**已存在**的文件前必须验证该文件在当前会话中已被读取，防止意外数据丢失；若未读取则拒绝并返回指示先读取文件的消息。新建文件无需预先读取。
...
- **FR-024**：`Edit` 工具在执行前必须进行陈旧检测：比较文件当前修改时间与 `readFileState` 中记录的修改时间。仅当当前修改时间晚于记录值（文件变新）时判定为可能被修改；此时若该次读取为完整读取（未提供 `offset` 和 `limit`）且文件当前内容哈希与记录值一致，则视为内容未变、允许继续；否则拒绝并返回"文件自上次读取后已被修改，请重新读取"的错误。分页读取（提供了 `offset` 或 `limit`）不享受内容兜底，mtime 变新即拒绝。
- **FR-025**：`Write` 工具对已存在文件执行与 `Edit` 相同语义的陈旧检测（见 FR-024）：仅当文件变新（当前 mtime 晚于 `readFileState` 记录值）时判定为可能被修改；此时若为完整读取（未提供 `offset` 和 `limit`）且当前内容哈希与记录值一致，则视为内容未变、允许继续；否则拒绝并返回"文件自上次读取后已被修改，请重新读取"的错误。新建文件不进行陈旧检测。
