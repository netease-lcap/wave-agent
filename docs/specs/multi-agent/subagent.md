...
6. **假设** 子代理的 frontmatter 用 YAML 块标量写多行值（`>`／`>-`／`|`／`|-` 等），**当** SDK 加载配置，**则** `description`／`model` 等字段必须是折叠／保留后的真实文本（不含 `>`、`|`、`-` 这类指示符），续行**不得**被解析成独立的 frontmatter 键；子代理列表与自动委托判据看到的描述都是解析后的结果
