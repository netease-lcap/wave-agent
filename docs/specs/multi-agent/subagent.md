...
- **FR-008**：系统必须将子代理限制为仅在其配置中指定的工具，如果未指定则继承所有工具（但 FR-025 中列出的工具始终被排除）
...
- **FR-025**：系统必须在子代理的 PermissionManager `instanceDeniedRules` 中始终拒绝 Task 管理工具（`TaskCreate`、`TaskGet`、`TaskUpdate`、`TaskList`），无论子代理配置中是否显式列出这些工具。子代理共享主代理的 TaskManager 实例，允许其访问将导致子代理能并发修改主代理的任务列表（创建/更新/删除任务、解析依赖），构成数据竞争隐患。此拒绝与 FR-012 禁止 `Agent` 工具采用同一机制（`instanceDeniedRules`），`shouldEnableTool` 对被拒绝工具返回 `false`，使其不会注册到子代理的 ToolManager。
