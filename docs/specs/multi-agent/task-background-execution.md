...
### 用户故事：后台任务弹窗键盘焦点圈（优先级：P2）

作为 IDE 插件（VS Code 扩展与 JetBrains 插件）用户，我希望 `/tasks` 弹窗的任务列表只提供一个 Tab stop，任务之间用方向键移动选中、Enter 打开详情，以便键盘用户可以选中任务查看输出/终止任务，而不是只能靠鼠标点击任务项。

**为什么是这个优先级**：当前任务项（`mcp-server-item`）不可聚焦（无 tabIndex），键盘用户无法选中任务查看详情，只能 Tab 到运行中任务的「停止」按钮。任务列表是单选列表语义，用 roving tabindex（列表一个 Tab stop + 方向键）对齐列表控件标准做法，也与消息列表焦点圈模式一致。

**独立测试**：打开 `/tasks` 弹窗，Tab 一步到达任务列表，↑/↓ 在任务间移动（高亮），Enter 打开选中任务详情，详情中 Tab 遍历「停止/返回列表/关闭」按钮，返回列表后方向键继续移动。

**验收场景**：

1. **假设** 弹窗中有任务列表，**当** Tab 焦点进入任务列表时，**则** 列表只提供一个 Tab stop（当前任务高亮），不再逐个 Tab 经过每个任务项的按钮。
2. **假设** 焦点在任务列表上，**当** 用户按 ↑/↓ 时，**则** 选中项在当前任务之间移动（高亮跟随），移动不触发任何操作。
3. **假设** 焦点在任务列表上，**当** 用户按 Enter/Space 时，**则** 打开选中任务的详情视图（与鼠标点击任务等效），焦点移入详情区可聚焦元素（如「停止」按钮）。
4. **假设** 详情视图打开，**当** 用户按 Tab 时，**则** 焦点在「停止」（运行中任务）/「返回列表」/「关闭」按钮间遍历；按「返回列表」后焦点回到任务列表选择器，方向键可继续移动。
5. **假设** 弹窗中无任务，**当** Tab 进入弹窗时，**则** 焦点直接落在「关闭」按钮上。
6. **假设** 弹窗关闭，**当** 关闭发生时，**则** 焦点归还打开弹窗前的触发位置（如消息输入框）。
...
- **任务列表 roving 与状态刷新**：任务状态变化（启动/完成/失败/终止）触发列表实时刷新时，选中任务 id 保留；若选中任务被移除（如任务列表清空），焦点回落到列表第一项，列表为空时落到「关闭」按钮。
