...
5. **假设**用户未认证且聊天头部显示"登录"按钮（更多按钮左侧），**当**用户点击该按钮时，**则**它与"更多"菜单中的"登录"项触发同一登录流程，二者行为一致。
...
**独立测试**：打开插件，验证聊天头部在未认证时显示"登录"按钮、已认证时消失；打开"更多"菜单验证包含设置、企业控制台和登录入口；欢迎页仅显示品牌引导、不含登录按钮。
...
5. **假设**欢迎页已显示且用户未认证，**当**聊天头部渲染时，**则**「更多」按钮左侧显示"登录"按钮；用户已认证或已配置 apiKey + baseURL 直连时该按钮不显示。
