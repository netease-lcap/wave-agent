...
1. **假设**用户打开"更多"菜单，**当**菜单渲染时，**则**包含"设置"、"企业控制台"、以及按认证状态切换的"登录/退出登录"入口（行为见 FR-028~031）。
2. **假设**用户点击"更多"菜单中的"设置"，**当**操作发生时，**则**打开插件配置对话框（见 agent-config.md 用户故事 10）。
...
- **FR-032**：IDE 插件"更多"菜单必须包含"设置"项，点击必须打开插件配置对话框（见 agent-config.md 用户故事 10）。
- **FR-033**：IDE 插件"更多"菜单必须包含"企业控制台"项，点击必须在系统浏览器中打开当前服务端地址对应的控制台页面。
- **FR-034**：当前会话无消息且初始化完成时，IDE 插件必须显示欢迎页；初始化未完成时必须显示加载动画。
- **FR-035**：欢迎页必须仅在用户未 SSO 登录且未配置 apiKey + baseURL 直连时显示登录引导文案与"登录"按钮（两者同时满足才隐藏）。
- **FR-036**：欢迎页必须在用户发送第一条消息后切换为正常对话视图。
- **FR-037**：欢迎页门控必须仅依据"当前会话是否有消息"，与认证状态解耦（与 FR-029 的退出登录不跳欢迎页行为一致）。
