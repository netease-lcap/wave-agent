...
**独立测试**：触发带超长 plan（30 行以上）的 `ExitPlanMode` 或大 diff 的 `Edit`/`Write` 确认，用 PgUp/PgDn 浏览全文，验证内容区滚动而选项列表位置不变。
...
4. **假设** 内容不超过可视高度，**则** 全部内容一次性显示，不出现滚动指示。
5. **假设** 选项列表在内容区下方，**则** 无论内容区滚动到哪一页，选项列表固定在底部始终可见，↑↓/Tab/Enter 只操作选项。
6. **假设** 出现新的确认（如确认队列下一项），**则** 内容区滚动位置重置到顶部。
...
- **详情内容超高怎么办？** 内容区（diff / 工具输入 JSON / plan）线性化为行，`scrollOffset` 状态 + PgUp/PgDn 翻页滚动（见「确认详情超高时滚动」），顶部/底部显示滚动指示；plan 以纯文本行渲染以便逐行滚动。
