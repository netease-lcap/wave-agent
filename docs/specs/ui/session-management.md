...
- **恢复会话必须读回标题并在当场追加回末尾（自愈的来源）**：恢复路径整读会话文件时必须收集 `custom-title` 作为内存值，并在加载完成后立即重追加到 EOF（验收场景 5 的第 3 项，对齐 Claude Code 的 `adoptResumedSessionFile`）——否则标题一旦滑出尾窗，退出时既没有内存值、尾窗也读不到，标题将永久退回首条消息；而且在本次会话退出之前，其它宿主在列表里也一直看不到标题。有这一环，任何「滑出」都只是窗口期而非数据丢失（文件里的条目 append-only 永不删；`/rewind` 的整文件重写是唯一会物理删除内容的路径，故它必须自己补回标题，见验收场景 5）。
- **`/rewind` 重写会连 metadata header 一起删掉（既有缺陷，本轮不修）**：`rewriteSessionFile` 只序列化消息，第一行的 `{"type":"metadata",...}` 连同末尾的 `custom-title` 一起被抹掉（实证：重写后文件只剩被截断的消息行，`readMetadata()` 返回 `null`、`readCustomTitle()` 返回 `undefined`）。本轮只补回标题（验收场景 5 第 5 项），header 的丢失会让该会话的 `createdAt` 在读取侧回退成「读取时刻」、`gitBranch` 变 `undefined`（`session.ts` 的 fallback 分支），属与标题无关的既有缺陷，单独跟踪。
