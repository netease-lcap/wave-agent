...
作为 IDE（VS Code 扩展 / JetBrains 插件 / 桌面端）用户，我希望通过 `/skills` 斜杠命令直接唤起设置页面并选中「技能」选项卡，以查看当前会话可见的所有技能（按来源分组），并能进入详情查看完整元数据，以便在不切换到 CLI 的情况下了解可用的技能。

**为什么是这个优先级**：CLI 已有 `/skills`；IDE 用户需要对等能力。设置页（批次 2）已提供「技能」导航项，`/skills` 复用设置页入口避免维护独立的技能对话框（2026-08-29 用户拍板：弹窗内容迁移到设置页选项卡）。

**独立测试**：在 IDE 中输入 `/skills`，验证打开设置页并选中「技能」选项卡，按来源分组列出技能；选中某项进入详情视图显示完整元数据；点击「返回」关闭设置页回到会话。

**验收场景**：

1. **假设** 用户处于 IDE 中（VS Code 扩展 / JetBrains 插件 / 桌面端），**当** 用户输入 `/skills` 并发送，**则** 打开设置页并选中「技能」选项卡（桌面端打开全页设置，IDE 打开编辑器区域设置标签页），不再弹出独立对话框。
2. **假设** 设置页「技能」选项卡已打开，**则** 展示当前会话可见的所有技能，按来源分组（内置 / 用户 / 项目 / 插件），每项展示名字、所属插件名（插件技能）、描述等关键字段。
