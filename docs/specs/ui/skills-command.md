...
### 用户故事：按来源 Tab 展示技能与项目分组（优先级：P1）

作为 GUI 用户，我希望设置页「技能」选项卡按来源以 Tab 形式展示技能，其中项目技能再按所属项目分组、每个项目一张卡片，以便快速定位某个来源或某个项目下的技能，并知道它属于哪一类（用户 / 项目 / 插件 / 内置）。

**为什么是这个优先级**：技能来源（用户/项目/插件/内置）决定其适用范围与可信度，Tab 化 + 项目分组让长列表可扫读，是对齐设计稿的基础形态（2026-08-29 用户拍板：按截图改造，项目技能按项目分组卡片展示）。

**独立测试**：在具备多种来源技能与多个项目技能的环境中打开设置页技能选项卡，验证四个来源 Tab 存在、项目技能按项目分组为卡片、每张卡片列出该项目下的技能并可执行管理操作。

**验收场景**：

1. **假设** 环境中存在插件、内置、用户、项目四种来源的技能，**当** 技能选项卡打开，**则** 顶部展示四个来源 Tab（插件技能 / 内置技能 / 用户技能 / 项目技能），点击切换显示对应来源的技能
2. **假设** 存在项目技能，**当** 用户位于「项目技能」Tab，**则** 技能按所属项目分组展示，每个项目一张卡片，卡片头显示项目名
3. **假设** 某项目卡片内存在多个技能，**当** 卡片展示，**则** 列出该项目下的所有技能（名称、描述），并提供「编辑」「删除」操作入口
4. **假设** 某项目技能可通过斜杠命令调用，**当** 卡片列表展示，**则** 技能名以斜杠命令样式（`/技能名`）展示，便于识别其调用方式
5. **假设** 项目技能来源可写（项目目录或用户目录），**当** 卡片头展示，**则** 提供「新增指令」入口用于新建该项目下的技能
...
2. **假设** 用户在「项目技能」Tab 某项目卡片内点击「新增指令」，**当** 输入框预填，**则** 预填项目级提示词并带上该项目名，如 `/settings 帮我在【项目名称】下新建一个技能：<技能名>，用于<场景>，内容是<做什么>`
...
5. **假设** 新建的是项目技能，**当** 技能页「项目技能」Tab 展示，**则** 新技能出现在所属项目的卡片列表中
...
- **项目技能按项目分组的项目归属如何确定？** 项目归属从技能文件路径推断（技能位于哪个工作目录的 `.wave/skills` / `.claude/skills` / `.agents/skills` 下，即属于哪个项目）；路径无法推断时归入「其他项目」分组
