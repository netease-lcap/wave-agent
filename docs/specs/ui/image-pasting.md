...
- 系统必须检测聊天输入中的 `Ctrl+V`（或平台等效操作）。
- 系统必须检查剪贴板是否包含图片数据。
- 系统必须支持在 macOS、Windows 和 Linux 上从剪贴板读取图片。
- 系统必须将剪贴板图片保存到临时文件。
- 系统必须为每张粘贴的图片生成唯一 ID。
- 系统必须在光标位置的输入文本中插入占位符 `[Image #<ID>]`。
- 系统必须在输入状态中维护附加图片列表。
- 系统必须在提交时解析输入文本中的图片占位符。
- 系统必须在发送给 AI 的消息中包含引用图片的文件路径和 MIME 类型。
- 系统必须在临时图片文件不再需要后清理它们（例如发送后或退出时）。
