...
2. **假设**一个正在运行的 bash 模式命令（`!ls`），**当**渲染时，**则**其输出实时更新。
...
**独立测试**：渲染含 200 条消息（每条带链接/按钮/代码块/表格）的会话，从消息输入框按 Shift+Tab 回到消息列表只需一步；方向键可在消息间移动；Enter 后 Tab 才进入该消息内部元素；全列表只有一个可聚焦站点（querySelectorAll 统计可聚焦元素不含列表内部元素，除激活的那条）；激活行内 markdown 代码块与超宽表格滚动容器可通过 Tab 聚焦并横向滚动，未激活时与链接/按钮一起被冻结为 `tabIndex -1`。
...
9. **假设** 激活行内含 markdown 代码块（`<pre>`）或超宽表格的横向滚动容器（`.markdown-table-wrapper`，均 `overflow-x: auto`），**当** Tab 遍历该行内部元素时，**则** 每个横向滚动容器是一个可聚焦站点（markdown renderer 输出 `tabindex="0"` DOM 属性），聚焦后可用方向键横向滚动溢出内容，无需鼠标。
10. **假设** 行未激活（含其内 markdown 代码块/表格容器），**当** Tab 循环经过消息列表时，**则** 这些滚动容器与行内链接/按钮一样被冻结为 `tabIndex -1`，不占 Tab 位；冻结后仍可被后续激活恢复为 0（roving 冻结通过 `data-roving` 哨兵记录成员资格，而非依赖 `[tabindex]` 重查——后者在冻结为 `-1` 后会丢失这些仅靠 `[tabindex]` 匹配的元素）。
11. **假设** 激活行内的最后一个可聚焦元素是代码块或表格容器，**当** 用户按 Tab 时，**则** 焦点返回消息列表选择器并失活该行（与链接/按钮行为一致）；在这些滚动容器上按 Escape 同样返回选择器。
...
- **消息内横向滚动容器（代码块/超宽表格）键盘可达性怎么办？** markdown renderer 给 `pre` 与表格滚动包裹层输出 `tabindex="0"`，纳入 roving 焦点圈：激活行后 Tab 可聚焦、方向键横向滚动，未激活时冻结为 `-1` 不占 Tab 位；roving 冻结以 `data-roving` 哨兵记录成员，冻结为 `-1` 后仍能恢复。
