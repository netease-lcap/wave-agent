...
3. **假设** 用户打开 Welcome 视图，**当**未认证或无会话时，**则**显示引导；登录后视图消失。
4. **假设** 用户处于未认证状态且当前会话没有任何可见消息（欢迎页显示），**当**欢迎页渲染时，**则**欢迎页品牌引导下方显示"登录"按钮与登录引导文案，点击后发起登录；登录成功后按钮消失。已有可见消息的对话中不显示欢迎页登录按钮（该场景下登录入口位于聊天"更多"菜单）。
