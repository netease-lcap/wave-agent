...
**独立测试**：对一条正在生成回复的会话运行 `wave daemon status <sessionId>`，验证输出包含 generating 状态与最近消息文本；对一条空闲会话运行则显示 idle 状态；对一条前台回合已结束但仍有后台任务/子 agent 在跑的会话运行，验证显示 generating（与 `wait` 同一判据，不显示 idle）。
...
11. **假设** 目标会话的前台回合已结束（`loading` 为 false），但它派出的后台 bash 任务 / 子 agent 仍在运行（或队列里还有未被主 agent 消费的消息），**当** 用户运行 `wave daemon status <sessionId>` 时，**则** 不得显示 idle（显示 generating，即「未空闲」）——`status` 与 `wait` 对「空闲」使用**同一判据**（`isLoading === false` 且无运行中的后台任务/子 agent 且无待处理消息），因此 `status` 显示 idle 时 `wait` 必然立刻退 0、`status` 显示 generating 时 `wait` 仍在阻塞；否则监控脚本会读到「`status` 说 idle、`wait` 却一直不返回」这种自相矛盾的取值（场景 2 括注里的「无后台任务、无排队消息」正是这一判据的一部分）。
...
**idle 的判据与 `wave -p`（print mode）逐条对齐：三项同时为假才算空闲**：`isLoading === false` **且** 无运行中的后台任务/子 agent（SDK `hasRunningBackgroundWork`）**且** 无待处理消息（SDK `hasPendingMessages`）。只看 `isLoading` 会把「主 agent 已结束本回合、但它派出去的后台 bash 任务或子 agent 还在跑」判成空闲并立刻退 0 报「已完成」（真实发生过：主 agent 把子调研 `run_in_background` 丢出去之后本回合即结束，`isLoading` 随即翻 false，后台任务的通知要**之后**才到达并开新一轮回合）。第三项 `hasPendingMessages` 是 **TOCTOU 防线**：最后一个后台任务翻成 `completed` 时，它的完成通知尚未被主 agent 取走（前两项此刻都已是假），而那条通知马上会触发新一轮回合——只看前两项会在这一瞬间假报完成（`print-cli` 的等待循环正是为堵这个窗口同时检查三项）。daemon 在 `listDaemonSessions` 的注册表条目里把这三项一并透出，`wait` 每次唤醒后重读注册表判定；后台任务起止时 daemon 也会推送 `backgroundTasksChange`（同样按 sessionId 过滤），与 `loadingChange` 一并作为本会话的唤醒源，保证「后台任务开跑/跑完」这类状态跃迁不会等到兜底 tick 才被发现。
...
16. **假设** 目标会话的前台回合已结束（注册表 `isLoading` 为 false），但它派出的**后台 bash 任务 / 子 agent 仍在运行**（SDK `hasRunningBackgroundWork` 为 true），或队列里还压着尚未被主 agent 消费的**后台任务完成通知**（SDK `hasPendingMessages` 为 true），**当** 用户运行 `wave daemon wait <sessionId>`（无论是否传 `--from-busy`）时，**则** 命令**不得**判成空闲退出——「空闲」的判据与 `wave -p`（print mode）一致，三项同时为假才算：`isLoading === false` **且** 无运行中的后台任务/子 agent **且** 无待处理消息。第三项堵住 TOCTOU：最后一个后台任务翻成 `completed` 时其完成通知尚未被取走，前两项此刻已全为假，而该通知马上会开新一轮回合——这一瞬间也不得退 0。因此这种会话要一直等到后台任务结束、主 agent 处理完通知并结束后续回合，才以退出码 0 退出并打印该**后续回合**的最终快照。唤醒同样事件驱动：除本会话的 `loadingChange` 外，daemon 在后台任务起止时推送的 `backgroundTasksChange` 也是本会话的唤醒源（同样先按 sessionId 过滤），waiter 被唤醒后重读注册表判定；低频兜底 tick 只是兜底，不得退化成高频轮询。**故意留着的长跑后台任务**（例如会话里长期挂着一个 dev server）会让 `wait` 一直等下去——这是预期行为，这种会话必须显式传 `--timeout`（见场景 6）。`send --wait` 不适用本判据：它按「自己那条 user message 的 id → 之后的回复」收尾，与空闲位无关，本轮不改。
...
- **`wait` 的退出码是契约**：`0` = 等到空闲、`3` = 会话挂起等待权限审批（立刻返回、不继续等）、`1` = 错误（daemon 连不上 / sessionId 不存在或等待期间被销毁 / `--timeout` 到点）。stdout 只承载最终快照（与 `status --lines N` 同格式），便于 `msg=$(wave daemon wait <id>)` 直接捕获汇报；进度提示走 stderr。等待由**本会话**的 `loadingChange` 与 `backgroundTasksChange`（后台任务起止时推送）唤醒（订阅早于 attach，订阅先按 sessionId 过滤再写状态），醒来后读注册表拿权威状态判定空闲——判据与 `wave -p`（print mode）一致：`isLoading === false` **且** 无运行中的后台任务/子 agent **且** 无待处理消息，三项同时为假才算空闲（只看 `isLoading` 会把「前台回合已结束、后台任务/子 agent 还在跑」误报成完成；`hasPendingMessages` 一项堵住最后一个后台任务刚翻 `completed`、完成通知尚未被消费的 TOCTOU 窗口）。推送只作唤醒、不作判定，因此别的会话的推送既不能结束等待也不能替 `--from-busy` 满足「忙」。只有**两类事实施信不了推送**、走低频兜底查询（默认 2 秒、不得快于 1 秒）：权限审批（挂起时 `loading` 保持 true，无法从 loading 变化中识别）与会话是否仍在 live 注册表中（销毁只伴随一次 `loadingChange:false`，与「本轮生成结束」同形，既不能当「已完成」也不能当「已消失」）——这是去问没有推送通道的事实，不是「每 N 秒查一次 status」的状态轮询。三者的判定顺序固定为**存在性 → 挂起审批(3) → 空闲(0) → 超时(1)**：销毁期间 destroy 内部的 abort 会清掉该会话的 loading（注册表读数与那条 `loadingChange:false` 都指向「已空闲」），空闲判定必须让位于存在性，否则会为一条正在被销毁的会话假报退出码 0（详见「阻塞等待会话空闲」场景 10）。`status` 保持「取一次快照、立刻返回」的契约不变，`wait` 是脚本轮询用法的正式替代而非 `status --watch`。
