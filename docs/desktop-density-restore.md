...
2. **C-02（代码行高整数化）本轮部分采纳** —— 行内 code / pre / pre code 已按用户 2026-09-10 指示取 20px（见文末「代码角色行高 20px」节）；**bash 命令与输出、写入预览、lsp-output 仍未统一**，属本候选剩余范围。
...
bash 命令与输出 **13/15.6px**、写入预览 **13/18px**、`lsp-output` **12/14.4px** 仍是各自的 1.2 / 1.38 系数，与本次 20px 刻度不一致。原候选 C-02 含这四类 + 日志，按 G4 口径一次收口更稳，待用户确认是否一并做。
