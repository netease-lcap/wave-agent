...
确认规格后，可选择进入 plan 模式制定技术方案（技术选型、架构设计、实现步骤）。方案全文展示在编辑器区域的计划预览中（桌面端 Plan 面板 / VS Code 预览面板 / JetBrains 编辑器标签页），确认框保持紧凑，方案经你批准后才开始编码。

![技术方案衔接](/screenshots/spec-sdd-confirm-plan.webp)
_技术方案衔接：单选「进入 plan 模式 / 跳过」_

![技术方案预览与批准](/screenshots/spec-sdd-plan-preview.webp)
_技术方案预览与批准：方案正文展示在计划预览面板中，确认框保持紧凑_

![计划批准交互](/screenshots/spec-sdd-plan-approve.webp)
_计划批准交互：确认框提供「批准并继续」「批准并自动接受后续修改」「提供反馈」等选项，批准后开始编码_
...
_迭代已有需求：AI 更新既有规格文件，任务列表显示「更新功能规格」进行中_
