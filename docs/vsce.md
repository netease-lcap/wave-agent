...
提出需求后，AI 自动编写功能规格说明（用户故事 + 验收场景），任务列表显示「编写功能规格」进行中；编写完成后通过单选弹窗请你决策——选择「直接实现」即直接进入编码、选择「制定技术方案」进入可选的技术方案阶段、选择「其他」时，AI 会按反馈更新规格后再次请求决策。

![规格编写阶段](/screenshots/spec-sdd-workflow-spec.webp)
_规格编写阶段：AI 产出规格说明，任务列表实时显示进度_

![规格决策](/screenshots/spec-sdd-confirm-spec.webp)
_规格决策：单选「直接实现 / 制定技术方案 / 其他」_
...
当需求是对已有功能的修改时，AI 不会另起新规格，而是直接更新对应的既有规格文件（沿用已有的规格目录与分组命名，新增用户故事和验收场景），任务列表显示「更新功能规格」进行中；更新完成后同样通过单选弹窗请你决策——选择「其他」时按反馈继续调整，选择「直接实现」或「制定技术方案」后按更新后的规格重新汇入相应阶段。
