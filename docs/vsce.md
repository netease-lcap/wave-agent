...
- **设置页**：命令面板中的「Wave: 打开设置」打开全页面设置（编辑器区域的设置标签页，`/config`、`/agents`、`/skills`、`/mcp` 唤起的是同一页面并自动选中对应选项卡，详见 [第 6.1 节](#agents-dialog)、[第 6.2 节](#skills-dialog) 与 [第 7.3 节](#mcp-integration)）：左侧导航分组（全局设置 / 个性化 / 项目设置 / AI 与扩展等），右侧为对应分组内容，点击「保存」生效。桌面端的全页设置见[桌面版文档](/desktop#设置)。
