...
提出需求后，AI 自动编写功能规格说明（用户故事 + 验收场景），任务列表显示「编写功能规格」进行中；编写完成后通过单选弹窗请你确认——选择"需要修改"时，AI 会按反馈更新规格后再次请求确认。

![规格编写阶段](/screenshots/spec-sdd-workflow-spec.webp)
_规格编写阶段：AI 产出规格说明，任务列表实时显示进度_

![规格确认](/screenshots/spec-sdd-confirm-spec.webp)
_规格确认：单选「确认通过 / 需要修改」_

**第二阶段（可选）：技术方案**

确认规格后，可选择进入 plan 模式制定技术方案（技术选型、架构设计、实现步骤）。方案全文在 VS Code 中展示于新标签页的计划预览面板（WebviewPanel，在对话面板旁打开）；确认框保持紧凑，方案正文不进入对话 Webview，方案经你批准后才开始编码。

![技术方案衔接](/screenshots/spec-sdd-confirm-plan.webp)
_技术方案衔接：单选「进入 plan 模式 / 跳过」_
...
当需求是对已有功能的修改时，AI 不会另起新规格，而是直接更新对应的既有规格文件（沿用已有的规格目录与分组命名，新增用户故事和验收场景），任务列表显示「更新功能规格」进行中；更新完成后同样通过单选弹窗请你确认——选择"需要修改"时按反馈继续调整，选择"确认通过"后按更新后的规格重新汇入技术方案（可选）与编码阶段。
