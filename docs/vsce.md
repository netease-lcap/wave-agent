...
- **`/agents`**：唤起设置页并选中「子代理」选项卡，查看当前会话中的子代理定义（详见 [第 6.1 节](#agents-dialog)）
- **`/skills`**：唤起设置页并选中「技能」选项卡，查看当前会话可见的技能，按来源分组并可查看详情（详见 [第 6.2 节](#skills-dialog)）

选择这些指令后会直接弹出对应弹窗（`/config`、`/agents`、`/skills` 除外——它们唤起设置页对应选项卡），无需按回车发送。
...
输入 `/agents` 斜杠命令会直接唤起设置页面并选中「子代理」选项卡（桌面端打开全页设置，VS Code / JetBrains 打开编辑器区域的设置标签页），查看当前会话中所有可见的子代理（subagent）定义。定义按来源分组展示——`内置 agents`、`用户 agents`、`项目 agents`、`插件 agents`，数据由扩展通过 `getSubagentConfigurations` 从 CLI 实时获取。
...
输入 `/skills` 斜杠命令会直接唤起设置页面并选中「技能」选项卡（桌面端打开全页设置，VS Code / JetBrains 打开编辑器区域的设置标签页），查看当前会话中所有可见的技能。技能按来源分组展示——`内置 skills`、`用户 skills`、`项目 skills`、`插件 skills`，数据由扩展通过 `getSkillMetadata` 从 CLI 实时获取。

![技能 - 列表](/screenshots/spec-skills-list.webp)
_设置页「技能」选项卡 - 按来源分组的技能列表_
...
用户可以自定义 AI 回复语言等基础参数，以适配不同的使用偏好。通过 `/config` 打开设置页「全局设置」选项卡进行管理；`/status`、`/plugin`、`/mcp` 三个斜杠命令分别唤起独立的弹窗。
...
- **设置页**：命令面板中的「Wave: 打开设置」打开全页面设置（编辑器区域的设置标签页，`/agents`、`/skills` 唤起的是同一页面并自动选中对应选项卡，详见 [第 6.1 节](#agents-dialog) 与 [第 6.2 节](#skills-dialog)）：左侧导航分组（全局设置 / 直连设置 / 个性化 / 项目设置 / AI 与扩展等），右侧为对应分组内容，点击「保存」生效。桌面端的全页设置见[桌面版文档](/desktop#设置)。
