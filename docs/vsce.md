...
启用 SDD 插件后，当你提出新的功能需求或修改需求时，AI 会引导你走完「规格 → 原型（可选）→ 技术方案（可选）→ 编码」的工作流：先产出功能规格说明并请你确认，再按需制作交互原型与技术方案，最后开始编码。各阶段之间的衔接均通过单选按钮完成，全程进度实时显示在任务列表中。
...
确认规格后，可选择先制作交互原型——原型仅实现前端界面，数据全部使用 mock，用于在编写业务代码之前验证界面与交互设计。仅当需求涉及前端界面时才会弹出此选择；不涉及前端界面的需求（如后端服务、CLI 工具）会自动跳过原型阶段。

![原型阶段衔接](/screenshots/spec-sdd-confirm-prototype.webp)
_原型阶段衔接：单选「制作原型 / 跳过」_

**第三阶段（可选）：技术方案**

原型完成后（或跳过原型时），可选择进入 plan 模式制定技术方案（技术选型、架构设计、实现步骤），方案经你批准后才开始编码。
...
方案批准后进入编码阶段。任务列表全程记录各阶段进度——规格、原型、技术方案标记为已完成、当前实现阶段标记为进行中；跳过的可选阶段不会出现在任务列表中。
