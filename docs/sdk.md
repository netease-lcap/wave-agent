...
- **池收敛**：沙箱里能调到的工具恰好是**代理本来就能直接调**的那一批（同一来源、同一拒绝过滤），所以 Exec 不是提权通道。收敛生效时不再逐条声明 `mcp__*` 工具，而是把这些工具渲染进 `Exec` 的描述（目录）；目录超出预算时按服务器轮转保留（避免靠后连接的服务器整段消失），并在末尾给出显式 `PARTIAL — X of Y` 提示并指向 `search`，不静默截断
