...
| `Exec`            | 在沙箱脚本里编排 MCP 调用（MCP 工具池非空时声明，并取代逐条扁平声明）                            |
...
| `title`     | string  | artifact 标题，仅 `.html` 生效：页面自带 `<title>` 永远优先，缺标签时用文件名兜底 |
...
- 标题与短名分离（对齐 Claude Code）：`.html` 的标题按「页面自带 `<title>` 优先 → `title` 参数 → 文件名兜底」解析，客户端总发非空 `title`，所以不写 `<title>` 的页面不会变成「Untitled artifact」；`.md` 保持文件名身份（渲染时把文件名注入 `<title>`，`title` 参数不适用于 Markdown）；`label` 只是本次发布的短名，不影响标题
...
- **池收敛**：沙箱里能调到的工具恰好是**代理本来就能直接调**的那一批（同一来源、同一拒绝过滤），所以 Exec 不是提权通道。收敛生效时不再逐条声明 `mcp__*` 工具，而是把工具目录作为**尾部消息增量播报**：池渲染结果变了就追加一条 `isMeta` 消息（池不变不重发、断开播报一次、重开后重新播报完整目录），`Exec` 自己的描述是与池无关的常量——因此连接一个 MCP 服务器不再改写 `tools[]`、不打断提示词前缀缓存。目录超出预算时按服务器轮转保留（避免靠后连接的服务器整段消失），并给出显式 `PARTIAL — X of Y` 提示，不静默截断
- **收敛条件**：只要可编目的 MCP 工具池非空就收敛（`Exec` 出现在 `tools[]` 里，且不再有逐条扁平声明）——不设数量门槛，与 opencode 一致；池为空时不声明 `Exec`。代价是极小池（1–7 个工具）下目录加编排开销比逐条声明略贵，换来的是形态不随池大小跳变。`enableExec: false` 或把 `Exec` 排除出工具集都会退回扁平声明，MCP 能力不受影响
...
### simplify — 代码简化与清理 {#skill-simplify}

审查已变更代码，从代码复用、代码质量和执行效率三个维度进行深度分析，并直接应用优化修改。

- **名称**: `simplify`
- **使用方式**: `/simplify`
- **特性**: 专注代码质量提升与重构，**不用于寻找功能性 Bug**（寻找 Bug 请使用 `/code-review`）

#### 执行阶段

1. **识别变更** — 通过 `git diff`（或 `git diff HEAD`）获取当前修改。若无 git 变更，则审查最近编辑过的文件。
2. **并行子代理审查** — 并行启动 3 个专用子代理：
   - **代码复用代理 (Code Reuse Review)**：搜索现有工具函数、公共模块或相邻逻辑，标记重复实现的新函数和可使用已有工具的内联逻辑。
   - **代码质量代理 (Code Quality Review)**：检查冗余状态、参数膨胀、复制粘贴变体、抽象泄漏、硬编码字符串、无意义的 JSX 嵌套和无效注释（仅保留解释 "WHY" 的非直观注释）。
   - **执行效率代理 (Efficiency Review)**：检查重复计算、多余文件 I/O、缺失并发、热路径膨胀、无保护的轮询更新、TOCTOU 反模式、内存/事件监听泄漏以及过度读取。
3. **自动修复** — 聚合 3 个子代理的建议并直接应用修改，最后向用户简要汇报修复成果（或确认代码已经很干净）。

### code-review — 代码审查 {#skill-code-review}

对当前分支的变更执行多维度的正确性 Bug 与代码质量审查，并自动将结果提交为 PR/MR 评论。

- **名称**: `code-review`
- **使用方式**: `/code-review [effort]`
- **选项**:
  - `low`：启动 2 个代理，可信度阈值 90（仅输出几乎确定的发现，噪声最少）
  - `medium`（默认）：启动 3 个代理，可信度阈值 80
  - `high`：启动 4 个代理，可信度阈值 70
  - `max`：启动 5 个代理，可信度阈值 60（覆盖面最广，可能含有较低可信度的发现）

#### 执行阶段

1. **收集上下文** — 获取 `git status`、自 `main` 合并基点以来的修改文件列表、提交日志和完整 diff 内容。若无变更则停止。
2. **并行审查** — 依据 `effort` 级别并行分发任务给子代理：
   - **Bug 扫描代理**（全级别）：浅层扫描，专注于大型、明显的功能 Bug，避免细枝末节。
   - **AGENTS.md 规范审计代理**（全级别）：审计变更是否符合仓库根目录及修改文件所在目录的 `AGENTS.md` 规范。
   - **Git 历史代理**（中级及以上）：结合 `git blame` 与修改行的历史上下文，评估代码变更带来的隐患。
   - **代码复用与质量代理**（高级及以上）：审查重复工具代码、参数膨胀、抽象泄漏、多余注释等质量坏味道。
   - **效率审查代理**（仅限 `max`）：深入评估性能损耗、缺失的并发和内存泄漏等。
3. **可信度评分 (Confidence Scoring)** — 为发现的每个缺陷启动独立代理，对照 PR diff 和 `AGENTS.md` 给出 0-100 的置信度评分（0=几乎肯定是误报；100=绝对确定且会高频触发）。评分同时过滤历史遗留缺陷、类型系统/编译器能自动捕获的问题、非修改行的缺陷等干扰项。
4. **结果交付** — 过滤低于阈值的分数。优先通过 GitHub CLI (`gh pr comment`) 或 GitLab CLI (`glab mr note`) 将审查意见以 **PR/MR 评论** 形式发表；若 CLI 不可用或无对应 PR/MR，则直接在对话中输出。

### deep-research — 深度主题调研 {#skill-deep-research}

全自动深度主题研究工作流，通过并行网页搜索、源文件抓取、对抗式论点校验与去重，最终生成带有引用的高可信度报告。

- **名称**: `deep-research`
- **使用方式**: 直接向 Agent 提问复杂调研需求，或明确要求深度研究
- **特性**: 通过 Workflow 工具编排多阶段流水线。若用户问题不够具体，运行前会先询问 2-3 个澄清问题以锁定调研范围。

#### 执行阶段

1. **拆解 (Scope)** — 将研究问题分解为 5 个不同的搜索维度（如背景概述、技术细节、最新进展、反面观点、实际应用等）。
2. **并行检索 (Search)** — 并行启动 5 个搜索代理，分别针对每个维度检索网络，提取关键发现与源 URL。
3. **抓取分析 (Fetch)** — 汇总并去重所有 URL，取前 15 个顶级源并行抓取，使用 AI 提取可证伪的核心论点（falsifiable claims）并附带引用来源。
4. **对抗性校验 (Verify)** — 对每个核心论点进行 3 票制对抗性校验。3 个独立 skeptical 代理专门寻找反驳证据，超过 2 票反驳的论点被剔除，其余判定为 Confirmed（确认）或 Contested（有争议）。
5. **综合报告 (Synthesize)** — 合并语义重复的论点，按可信度排序，生成结构化深度报告：
   - 执行摘要 (Executive summary)
   - 按可信度排序且带引用的核心结论 (Key findings with citations)
   - 争议结论与前置条件 (Contested findings with caveats)
   - 不确定性区域 (Areas of uncertainty)
   - 总结 (Conclusion)
...
- **要点**: 会话经 `wave daemon create --worktree` 建立在隔离 worktree 中，默认 `bypassPermissions`（不产生审批）；`wave daemon send` 默认异步派单，进度与最终汇报用 `wave daemon status [--lines N]` 查看（默认只展示最后 1 条消息，`--lines 0` 只看状态行）；盯会话改用 `wave daemon wait <sessionId>` 阻塞等到空闲并直接把最终快照打到 stdout（退出码 0=空闲 / 3=挂起等审批 / 1=错误，含等待期间该会话被其它客户端销毁——不挂住也不误报完成；三种终止已覆盖无人值守监控的全部情形，替代自建 shell 轮询脚本，`status` 仍是「取一次快照、立刻返回」）；子命令语法细节见 [Daemon 客户端命令](cli.md#daemon-commands)
- **主要经验**: 被中断的 `send` 不等于消息没送达（先 `status` 复核再决定是否重发）；紧跟异步 `send` 之后 `wait` 要加 `--from-busy`（否则 turn 尚未开始的 stale `loading:false` 会让 wait 提前结束）；改范围先 `abort`；审批刷屏的根因是 daemon 进程重启后权限模式回落，用 `respond <id> --allow --mode bypassPermissions` 一步恢复；`destroy --remove-worktree` 是两步操作，中断会部分执行
