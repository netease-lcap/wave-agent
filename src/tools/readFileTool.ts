...
export const readFileTool: ToolPlugin = {
  ...
  execute: async (
    ...
    const actualShouldReadEntireFile =
      typeof shouldReadEntireFile === "boolean" ? shouldReadEntireFile : false;
    ...
    if (!actualShouldReadEntireFile) {
      ...
      if (!hasStartLine && !hasEndLine) {
        ...
        try {
          ...
          logger.warn(
            `Line numbers not provided, using default range: ${actualStartLine}-${actualEndLine}`,
          );
        } catch (error) {
          return {
            success: false,
    ...
    try {
      ...
      if (actualEndLine > totalLines) {
        logger.warn(
          `End line ${actualEndLine} exceeds file length ${totalLines}, adjusted to ${adjustedEndLine}`,
        );
      }
      ...
      if (actualStartLine > adjustedEndLine) {
        return {
          ...
          content: "",
          error:
            "start_line_one_indexed must be less than or equal to end_line_one_indexed_inclusive",
        };
      }
      ...
      if (
        ...
        if (finalRequestedLines < 200) {
          ...
          // 使用调整后的范围
          const adjustedSelectedLines = lines.slice(
            adjustedStartLine - 1,
            adjustedFinalEndLine,
          );
  ...
  formatCompactParams: (params: Record<string, unknown>) => {
    ...
    const startLine = params.start_line_one_indexed as number;
    const endLine = params.end_line_one_indexed_inclusive as number;

    if (!targetFile) return "";

    if (startLine && endLine) {
      return `${targetFile}:${startLine}-${endLine}`;
    } else if (startLine) {
      return `${targetFile}:${startLine}+`;
    } else if (endLine) {
      return `${targetFile}:1-${endLine}`;
    }
