...
The general-purpose agent MUST be used for all planning phases. Every phrase and decision during the planning process MUST be validated or generated by the general-purpose agent to ensure comprehensive codebase understanding and technical accuracy. Furthermore, tasks defined in `tasks.md` MUST be delegated to specialized subagents (e.g., `typescript-expert`, `vitest-expert`) whenever possible to reduce context costs of the main agent and leverage specialized expertise.

**Rationale**: The general-purpose agent's ability to perform multi-step research and analyze complex system architectures ensures that implementation plans are grounded in the actual state of the codebase. Delegating tasks to subagents optimizes context usage and ensures high-quality implementation through specialization.
