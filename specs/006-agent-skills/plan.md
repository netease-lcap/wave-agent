...
**Branch**: `006-agent-skills` | **Date**: 2024-12-19 | **Spec**: [spec.md](./spec.md)
**Input**: Feature specification from `/specs/006-agent-skills/spec.md`

**Note**: This template is filled in by the `/speckit.plan` command. See `.specify/templates/commands/plan.md` for the execution workflow.

## Summary

Implement Agent Skills support - a system that allows users to package expertise into discoverable capabilities. Skills consist of SKILL.md files with YAML frontmatter containing name and description, plus optional supporting files. The system must autonomously decide when to invoke skills based on user requests and skill descriptions. Additionally, skills can be explicitly invoked by users using a slash command syntax (e.g., `/my-skill arg1 arg2`), supporting parameter substitution (e.g., `$1`, `$ARGUMENTS`) and bash command execution using `!`command`` (inline) or triple-backtick-! blocks (replaced by raw stdout, capped at 30K chars with safe replacement). If no `$ARGUMENTS` or `$n` placeholders are present, all arguments are automatically appended to the end of the content. This includes creating a new Skill tool that reuses the existing markdown meta parser from custom commands, supports both personal (~/.wave/skills/) and project (.wave/skills/) skills, and enables progressive loading of supporting resources. The system also provides control flags like `disable-model-invocation` to prevent AI from automatically triggering a skill and `user-invocable` to control visibility in the slash command menu.
