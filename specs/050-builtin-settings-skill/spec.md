...
**Feature Branch**: `050-builtin-settings-skill`  
**Created**: 2026-03-18  
...
- **FR-004**: The `settings` skill MUST provide a guide on how to configure Wave, covering all supported fields in `settings.json` (`hooks`, `env`, `permissions`, `enabledPlugins`, `language`, `autoMemoryEnabled`, `autoMemoryFrequency`, `models`) and other configuration files (`.mcp.json` for MCP servers including the `type` transport field, `.wave/rules/` for memory rules, `.wave/skills/` for custom skills, and `.wave/agents/` for subagents).
