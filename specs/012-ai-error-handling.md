...
作为用户，我希望代理能够应对临时 API 速率限制（429 错误），这样我的工作不会被瞬态网络或 API 问题中断。

**验收场景**：

1. **假设**AI API 返回 429 错误，**当**代理发出请求时，**则**它应该使用指数退避自动重试请求（最多 5 次重试）
...
- **FR-005**：系统必须使用指数退避重试 429 错误（最多 5 次重试）
...
- **FR-010**：系统必须使用指数退避重试瞬态服务器错误（HTTP 500、502、503、504），使用与 429 错误相同的重试策略
