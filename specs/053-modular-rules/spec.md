# Feature Specification: Modular Memory Rules
...
### User Story 1 - Project-Level Modular Memory Rules (Priority: P1)
...
### User Story 2 - Path-Specific Memory Rules (Priority: P1)
...
### User Story 3 - Memory Rule Organization in Subdirectories (Priority: P2)
...
### User Story 4 - User-Level Memory Rules (Priority: P2)
...
- **FR-002**: System MUST support YAML frontmatter in memory rule files with a `paths` field containing a list of glob patterns.
- **FR-003**: System MUST apply path-specific memory rules if *any* file currently in the context window (read or modified) matches the glob patterns.
- **FR-011**: If multiple files are in context, the system MUST activate the union of all matching memory rules.
...
- **FR-007**: System MUST load user-level memory rules from `~/.wave/rules/`.
- **FR-008**: System MUST prioritize project-level memory rules over user-level memory rules when they are loaded.
- **FR-009**: Memory rules without a `paths` field MUST be loaded unconditionally.
- **FR-010**: Modular memory rules MUST have the same priority as `AGENTS.md` within the project context.
