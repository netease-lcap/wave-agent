...
- **Public surface**: the barrel (`import … from "wave-agent-sdk"`) is for the CLI, which embeds the agent in-process. The other hosts (desktop main process, VS Code extension host, and JetBrains once it stops shelling out) drive the agent over JSON-RPC and must import the narrow entries only — `wave-agent-sdk/types` for types, `/constants` for shared literals, `/stdio` for the RPC layer. A barrel import from a host drags the whole agent runtime (tools, providers, transitive deps — including ones that do work while their module body evaluates) into that host's bundle: `be05c42be` added two import lines for a shared string and grew the desktop main bundle by 2.2 MB, and it is what made desktop 1.2.5 crash before `whenReady`. Enforced by `tests/noSdkBarrelImport.test.ts` in `packages/desktop` and `packages/vscode`; add shared values to the narrow entry instead of reaching for the barrel.
