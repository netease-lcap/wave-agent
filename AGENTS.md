...
- **Host transport (`src/stdio/`)**: `StdioAgent` mirrors the `Agent` API over JSON-RPC with a `sessionId` on every call; `NotificationRouter` demuxes notifications by session. Re-exported by the `wave-agent-sdk/host` entry (also available as `wave-agent-sdk/stdio`) and used by the VS Code extension and desktop app.
