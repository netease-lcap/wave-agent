...
describe("StdioAgent", () => {
  ...
  it("sends initialize request, caches returned state, and registers with router", async () => {
    ...
    expect(client.request).toHaveBeenCalledWith("initialize", {
      workdir: "/workspace",
    });
  ...
  it("getPermissionMode returns cached value", () => {
    ...
    (agent as unknown as { permissionMode: PermissionMode }).permissionMode =
      "bypassPermissions";
  ...
  it("errorBlockAdded forwards error string to callback", () => {
    ...
    agent.handleNotification("errorBlockAdded", {
  ...
  it("loadingChange updates latestTotalTokens and calls callback", () => {
    ...
    agent.handleNotification("loadingChange", {
      loading: true,
  ...
  it("workdirChange updates cached workingDirectory and calls callback", () => {
    ...
    agent.handleNotification("workdirChange", {
      workdir: "/repo/.wave/worktrees/feat-a",
    });
  ...
  it("permissionRequest forwards requestId and context to callback", () => {
    ...
    agent.handleNotification("permissionRequest", {
      requestId: "req-1",
      context,
    });
