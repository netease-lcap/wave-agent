...
describe("NotificationRouter", () => {
  ...
  it("dispatches notification to registered agent by sessionId", () => {
    ...
    getHandler(client, "userMessageAdded")(
      { message: { id: "m1", role: "user", timestamp: "", blocks: [] } },
      "s1",
    );

    expect(agent.handleNotification).toHaveBeenCalledWith("userMessageAdded", {
      message: { id: "m1", role: "user", timestamp: "", blocks: [] },
    });
  });
  ...
  it("dispatches to the correct agent when multiple are registered", () => {
    ...
    router.register(
      "s1",
      agent1 as unknown as Parameters<NotificationRouter["register"]>[1],
    );
    router.register(
      "s2",
      agent2 as unknown as Parameters<NotificationRouter["register"]>[1],
    );

    getHandler(client, "tasksChange")({ tasks: [] }, "s2");

    expect(agent1.handleNotification).not.toHaveBeenCalled();
    expect(agent2.handleNotification).toHaveBeenCalledWith("tasksChange", {
      tasks: [],
    });
  });
  ...
  it("notification with sessionId for a global method goes to the agent (sessionId wins)", () => {
    ...
    expect(agent.handleNotification).toHaveBeenCalledWith("authUrl", {
      url: "https://example.com/auth",
    });
  ...
  it("sessionIdChange rekeys the agent from old to new sessionId", () => {
    ...
    expect(agent.handleNotification).toHaveBeenCalledWith("sessionIdChange", {
      sessionId: "new-id",
    });
