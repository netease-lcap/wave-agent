...
if (production) {
  ...
  execSync("pnpm -F wave-webview run compile -- --production", {
    stdio: "inherit",
  });
} else if (!fs.existsSync(srcDir)) {
  console.log(
    "[sync:webview] webview dist not found, building wave-webview...",
  );
...
console.log(
  `[sync:webview] Copied ${files.length} files to webview/dist/ (${files.join(", ")})`,
);
