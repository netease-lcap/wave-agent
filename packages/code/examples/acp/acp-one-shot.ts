...
/**
 * One-shot command using @agentclientprotocol/sdk and wave --acp
 */
async function runOneShot() {
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "acp-one-shot-"));
  ...
  try {
    ...
    await connection.initialize({
      protocolVersion: 1,
      clientInfo: { name: "one-shot-client", version: "1.0.0" },
    });
    ...
    console.log("One-shot example completed successfully!");
  } catch (error) {
