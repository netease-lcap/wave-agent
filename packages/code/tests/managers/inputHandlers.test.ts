...
describe("inputHandlers", () => {
  ...
  describe("handlePasteInput", () => {
    ...
    it("should use isPastingRef over state.isPasting when provided", () => {
      const isPastingRef = { current: false };
      const pasteBufferRef = { current: "" };
      // State says isPasting=false but ref says true (simulating ref updated before state)
      ...
      // Should NOT dispatch START_PASTE even though state.isPasting is false
      expect(dispatch).not.toHaveBeenCalledWith({
        type: "START_PASTE",
    ...
    it("should set refs on START_PASTE when refs provided and not yet pasting", () => {
      const isPastingRef = { current: false };
      const pasteBufferRef = { current: "" };
      const state = { ...initialState, isPasting: false, cursorPosition: 3 };
      ...
      expect(dispatch).toHaveBeenCalledWith({
        type: "START_PASTE",
        payload: { buffer: "first chunk", cursorPosition: 3 },
      });
    });

    it("should accumulate paste chunks via refs across multiple calls", () => {
      const isPastingRef = { current: false };
      const pasteBufferRef = { current: "" };
      const state = { ...initialState, isPasting: false, cursorPosition: 0 };

      // Simulate rapid sequential paste events (as happens in terminal)
      handlePasteInput(
        state,
        dispatch,
        callbacks,
      ...
      // state is still isPasting=false (React hasn't updated), but ref is true
      handlePasteInput(
      ...
      expect(isPastingRef.current).toBe(true);
      expect(pasteBufferRef.current).toBe("chunk1chunk2chunk3");
      // Only one START_PASTE should have been dispatched
      const startPasteCalls = (
        dispatch as ReturnType<typeof vi.fn>
      ).mock.calls.filter(
        (call: InputAction[]) => call[0].type === "START_PASTE",
      );
      expect(startPasteCalls).toHaveLength(1);
      // Two APPEND_PASTE_BUFFER calls
      const appendCalls = (
        dispatch as ReturnType<typeof vi.fn>
      ).mock.calls.filter(
        (call: InputAction[]) => call[0].type === "APPEND_PASTE_BUFFER",
      );
      expect(appendCalls).toHaveLength(2);
      expect(appendCalls[0][0].payload).toBe("chunk2");
      expect(appendCalls[1][0].payload).toBe("chunk3");
    });

    it("should fall back to state.isPasting when refs not provided", () => {
      const state = { ...initialState, isPasting: true };
      handlePasteInput(state, dispatch, callbacks, "more text");
      expect(dispatch).toHaveBeenCalledWith({
        type: "APPEND_PASTE_BUFFER",
    ...
    it("should handle paste with carriage return", () => {
      ...
      expect(dispatch).toHaveBeenCalledWith({
        type: "START_PASTE",
        payload: { buffer: "a\rb", cursorPosition: 0 },
      });
    });
  });
  ...
  describe("handleInput", () => {
    ...
    it("should pass isPastingRef through to handleNormalInput for paste detection", async () => {
      const isPastingRef = { current: false };
      const pasteBufferRef = { current: "" };
      const state = { ...initialState, cursorPosition: 0 };
      const key = {} as Key;

      // Simulate a multi-char paste event
      const result = await handleInput(
        state,
        dispatch,
        callbacks,
      ...
      expect(result).toBe(true);
      expect(isPastingRef.current).toBe(true);
      expect(pasteBufferRef.current).toBe("pasted text");
      expect(dispatch).toHaveBeenCalledWith({
        type: "START_PASTE",
        payload: { buffer: "pasted text", cursorPosition: 0 },
      });
    ...
    it("should accumulate paste chunks via refs across multiple handleInput calls", async () => {
      const isPastingRef = { current: false };
      const pasteBufferRef = { current: "" };
      const key = {} as Key;

      // Simulate rapid sequential paste events
      await handleInput(
        initialState,
        dispatch,
        callbacks,
    ...
    it("should work without refs (backward compatibility)", async () => {
      const key = {} as Key;
      const result = await handleInput(
        initialState,
        dispatch,
