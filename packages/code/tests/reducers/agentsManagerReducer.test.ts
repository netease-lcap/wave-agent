...
import {
  ...
  AgentsManagerState,
} from "../../src/reducers/agentsManagerReducer.js";
import { Key } from "ink";

describe("agentsManagerReducer", () => {
  ...
  it("should handle MOVE_UP", () => {
    const state: AgentsManagerState = { ...initialState, selectedIndex: 1 };
    const result = agentsManagerReducer(state, { type: "MOVE_UP" });
    expect(result.selectedIndex).toBe(0);
  });

  it("should clamp MOVE_UP at zero", () => {
    const result = agentsManagerReducer(initialState, { type: "MOVE_UP" });
    expect(result.selectedIndex).toBe(0);
  });

  it("should handle MOVE_DOWN", () => {
    const result = agentsManagerReducer(initialState, {
      type: "MOVE_DOWN",
      itemCount: 5,
    });
  ...
  it("should clamp MOVE_DOWN at itemCount - 1", () => {
    const state: AgentsManagerState = { ...initialState, selectedIndex: 4 };
    const result = agentsManagerReducer(state, {
      type: "MOVE_DOWN",
      itemCount: 5,
    });
  ...
  it("should handle CLEAR_PENDING_EFFECT", () => {
    const state: AgentsManagerState = {
      ...initialState,
  ...
  describe("HANDLE_KEY", () => {
    ...
    it("should return to list mode on return in detail mode", () => {
      const state: AgentsManagerState = { ...initialState, viewMode: "detail" };
      const result = agentsManagerReducer(state, {
        type: "HANDLE_KEY",
        input: "",
        key: { return: true } as unknown as Key,
    ...
    it("should move selection up on upArrow in list mode", () => {
      const state: AgentsManagerState = { ...initialState, selectedIndex: 2 };
      const result = agentsManagerReducer(state, {
        type: "HANDLE_KEY",
        input: "",
