...
function makeRequest(module: 'http' | 'https') {
  return vi.fn((url: string, _options: unknown, callback: (res: unknown) => void) => {
...
const GITHUB_LATEST = 'https://api.github.com/repos/netease-lcap/wave-agent/releases/latest';
...
describe('checkForUpdate via GitHub', () => {
  it('returns UpdateInfo when the latest release is newer', async () => {
    h.handler = () => ({
      statusCode: 200,
      body: JSON.stringify({ tag_name: 'v0.20.0', html_url: 'https://github.com/release/v0.20.0', body: 'notes' }),
    });
  ...
  it('falls back to GitHub when the manifest check fails', async () => {
    h.handler = (url) => {
      ...
      if (url === GITHUB_LATEST) {
        return { statusCode: 200, body: JSON.stringify({ tag_name: 'v0.20.0', html_url: 'gh' }) };
      }
      return new Error(`unexpected url: ${url}`);
    };
