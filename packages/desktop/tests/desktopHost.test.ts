...
describe("background session toasts", () => {
  /** Give the bound agent a registered session entry (title drives toast copy). */
  function seedSession(
    store: ReturnType<typeof createHost>["store"],
    sessionId: string,
    title: string,
  ) {
    const agent = lastAgent();
  ...
  /** Open a second pane (spawns + focuses its agent) — pane-1's agent goes background. */
  ...
  it("toasts a confirmation request from a pane-less background session with a 查看 action", async () => {
    ...
    const toasts = confirmationToasts(sent);
    expect(toasts).toHaveLength(1);
    expect(toasts[0].toast.message).toBe(
      "会话「任务A」需要确认：命令执行待确认",
    );
  ...
  it("announces a waiting session once per cycle, then again once all requests resolve", async () => {
    ...
    agent1.callbacks.onPermissionRequest("req-2", {
      toolName: "Edit",
      toolInput: { file_path: "/x.ts" },
    });
...
describe("history popup session list (desktop-app.md 历史对话弹窗)", () => {
  ...
  it("recognizes CLI-created worktree sessions by their working directory", async () => {
    const { host, sent } = await readyHost();
    // A session created by the CLI inside a wave-managed worktree has no
    // desktop index entry — it is recognized by its `.wave/worktrees` path:
    // repo root for display, worktree tag, git branch preserved.
    h.allSessions = [
      {
        id: "s-cli-wt",
        sessionType: "main",
        workdir: "/work/repo/.wave/worktrees/gentle-pike",
        branch: "worktree-gentle-pike",
        firstMessage: "CLI worktree session",
        lastActiveAt: new Date("2023-12-01T11:00:00Z"),
        latestTotalTokens: 10,
      },
      {
    ...
    const sessions = sent("updateSessions").at(-1)?.sessions;
    const wt = sessions?.find((s) => s.id === "s-cli-wt");
    expect(wt).toMatchObject({
      id: "s-cli-wt",
      workdir: "/work/repo", // repo root, not the worktree path
      worktree: true,
      branch: "worktree-gentle-pike",
    });
    const cli = sessions?.find((s) => s.id === "s-cli");
    expect(cli?.workdir).toBe("/work/other-project");
    expect(cli?.worktree).toBeUndefined();
  });

  it("restores a CLI-created worktree session into the worktree dir and registers it with worktree metadata", async () => {
    const { host, store, sent } = await readyHost();
    // The popup scan fills the in-memory worktree map for index-less sessions.
    h.allSessions = [
      {
        id: "s-cli-wt",
        sessionType: "main",
    ...
    expect(
      sent("updateSessions")
        .at(-1)
        ?.sessions.find((s) => s.id === "s-cli-wt")?.workdir,
    ).toBe("/work/repo");
    h.existingPaths.add("/work/repo/.wave/worktrees/gentle-pike");
    const before = h.agentInstances.length;

    // The list shows the repo root; the restore must target the worktree path
    // (where the transcript lives), never the repo root.
    await host.handleWebviewMessage({
      command: "desktopSelectSession",
      workdir: "/work/repo",
      sessionId: "s-cli-wt",
    ...
    await vi.waitFor(() => {
      ...
      expect(lastAgent().restoreSession).toHaveBeenCalledWith("s-cli-wt");
    });
    // The restored session joins the index grouped under the repo root with
    // worktree metadata — matching desktop-created worktree sessions.
    await vi.waitFor(() => {
      const entry = store
        .getSessionIndex()
        .find((e) => e.sessionId === "s-cli-wt");
      expect(entry?.workdir).toBe("/work/repo");
      expect(entry?.worktree).toMatchObject({
        path: "/work/repo/.wave/worktrees/gentle-pike",
        repoRoot: "/work/repo",
      });
    });
  });
