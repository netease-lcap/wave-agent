...
describe("parseConnectionString", () => {
  it("parses user@hostname", () => {
    expect(parseConnectionString("ssh dev@myhost")).toEqual({
      host: "myhost",
      hostName: "myhost",
      user: "dev",
    });
  ...
  it("parses hostname with optional leading ssh", () => {
    expect(parseConnectionString("myhost")).toEqual({
      host: "myhost",
      hostName: "myhost",
    });
    expect(parseConnectionString("ssh myhost")).toEqual({
      host: "myhost",
      hostName: "myhost",
    });
  });
...
describe("addSshHost", () => {
  ...
  it("throws on an unparsable connection string", () => {
    expect(() => addSshHost("ssh -i key.pem user@host", CONFIG)).toThrow(
      "无法解析",
    );
  });
...
describe("buildSshTunnelArgs", () => {
  it("builds an -N unix-socket forward with ExitOnForwardFailure", () => {
    expect(
      buildSshTunnelArgs(
        "prod",
        "/tmp/wave-daemon-prod.sock",
        "/home/u/.wave/daemon.sock",
      ),
    ).toEqual([
      ...SSH_BASE_OPTIONS,
      "-o",
      ...
      "-N",
      "-L",
      "/tmp/wave-daemon-prod.sock:/home/u/.wave/daemon.sock",
      "prod",
    ]);
  });
  ...
  it("keeps ServerAlive probes tight enough to detect a half-open tunnel after sleep", () => {
    ...
    const aliveInterval = Number(
      args.find((a) => a.startsWith("ServerAliveInterval="))?.split("=")[1],
    );
    const aliveCountMax = Number(
      args.find((a) => a.startsWith("ServerAliveCountMax="))?.split("=")[1],
    );
