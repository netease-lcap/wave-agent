...
import { PortForwardManager, rewriteForwardedUrl } from '../src/main/portForward';

const lastChild = (): FakeChild => h.spawn.mock.results.at(-1)?.value as FakeChild;
...
describe('PortForwardManager.acquire', () => {
  it('passes local hosts through without spawning ssh', async () => {
    ...
    await expect(manager.acquire(LOCAL_HOST, 'http://localhost:5173/x')).resolves.toEqual({
  ...
  it('rejects non-http(s) schemes and unparseable URLs', async () => {
    const manager = new PortForwardManager();
    await expect(manager.acquire('prod', 'file:///etc/passwd')).rejects.toThrow('仅支持 http/https');
    await expect(manager.acquire('prod', 'not a url')).rejects.toThrow('无法解析链接');
    manager.dispose();
  });

  it('spawns ssh -N -L bound to 127.0.0.1 and returns the rewritten URL', async () => {
    const manager = new PortForwardManager();
    const result = await manager.acquire('prod', 'http://localhost:5173/proto?x=1#top');
    expect(h.spawn).toHaveBeenCalledWith(
      'ssh',
      [...SSH_BASE_OPTIONS, '-N', '-L', '127.0.0.1:5173:localhost:5173', 'prod'],
      { stdio: 'ignore' },
    );
...
describe('rewriteForwardedUrl', () => {
  it('pins the host to 127.0.0.1 and preserves path/search/hash', () => {
    expect(rewriteForwardedUrl('http://localhost:5173/app?a=1#sec', 5173)).toBe('http://127.0.0.1:5173/app?a=1#sec');
