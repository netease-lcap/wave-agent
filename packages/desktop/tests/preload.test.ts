...
describe('preload', () => {
  ...
  function exposedApi() {
    const call = vi.mocked(contextBridge.exposeInMainWorld).mock.calls.find(
      ([key]) => key === 'acquireVsCodeApi',
    );
  ...
  it('exposes acquireVsCodeApi emulating the VS Code webview API', () => {
    ...
    expect(ipcRenderer.send).toHaveBeenCalledWith(WEBVIEW_CHANNEL, { command: 'webviewReady' });
  ...
  it('exposes waveHostType as desktop', () => {
    expect(contextBridge.exposeInMainWorld).toHaveBeenCalledWith('waveHostType', 'desktop');
  });
  ...
  it('forwards host messages into the page via window.postMessage', () => {
    ...
    const listener = vi.mocked(ipcRenderer.on).mock.calls.find(([channel]) => channel === HOST_CHANNEL)?.[1];
    expect(listener).toBeDefined();
    (listener as (event: unknown, message: unknown) => void)({}, { command: 'setInitialState' });
