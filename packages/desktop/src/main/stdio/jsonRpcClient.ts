 ...
 *
 * Shared by StdioClient (child-process stdio) and SocketClient (unix-socket
 * tunnel to a remote wave daemon). Subclasses own the transport and hook in:
 * - `writeLine(message)` writes one JSON line to the peer.
 * - `attachReadable(readable)` wires the inbound half (stdout / socket).
