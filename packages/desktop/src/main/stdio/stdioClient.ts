...
export class StdioClient extends JsonRpcClient {
  ...
  constructor(
    ...
    // The bundled CLI is a `.mjs` script executed by the Electron-bundled
    // Node runtime (process.execPath + ELECTRON_RUN_AS_NODE=1). A script is
    // spawned directly with an args array and no shell — cleaner than the
    // old `wave.cmd` shim path. WAVE_CLI_PATH overrides may point at other
    // executables (e.g. a `wave.cmd` shim), which keep the legacy shell
    // behaviour: Node (since the CVE-2024-27980 patch) refuses to spawn
    // `.cmd`/`.bat` without a shell, and with `shell: true` the executable
    // must be pre-quoted so a path containing spaces is not split.
    ...
    const spawnEnv = isScript
      ? { ...process.env, ...env, ELECTRON_RUN_AS_NODE: "1" }
      : { ...process.env, ...env };
