#!/usr/bin/env node
/**
 * prerun — rebuild the shared webview, then kill leftover dev Electron
 ...
 * it here keeps `pnpm run run` honest without having to remember the build
 ...
 * `pnpm run run` silently exit 0 (focus jumps to the old instance and edits
...
function buildWebview() {
  ...
  if (r.error) {
    console.error(`[prerun] failed to spawn pnpm: ${r.error.message}`);
    process.exit(1);
  }
  if (r.status !== 0) {
    console.error(`[prerun] webview build exited with status ${r.status}`);
    process.exit(r.status ?? 1);
  }
  console.log("[prerun] webview rebuilt");
}
...
if (victims.length === 0) {
  console.log("[prerun] no stale dev instance found");
  process.exit(0);
}
...
for (const v of victims) {
  try {
    ...
    console.log(`[prerun] killed ${v.pid}  ${v.snippet}`);
  } catch {
