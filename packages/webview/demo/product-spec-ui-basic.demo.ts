...
test.describe("Product Specification Screenshots - UI Basic", () => {
  test("capture basic UI features", async ({ webviewPage }) => {
    ...
    await screenshotWebp(
      webviewPage,
      "../../docs/public/screenshots/spec-welcome-login.webp",
    );
    ...
    await webviewPage.waitForSelector(
      '.context-tag-container[data-is-selection="true"]',
    );
    await elementScreenshotWebp(
      webviewPage.locator(".input-container"),
      "../../docs/public/screenshots/spec-selection-inline-tag.webp",
    );
    ...
    await elementScreenshotWebp(
      webviewPage.locator(".input-container"),
      "../../docs/public/screenshots/spec-input-empty.webp",
    );
    ...
    await elementScreenshotWebp(
      webviewPage.locator(".input-container"),
      "../../docs/public/screenshots/spec-input-focus.webp",
    );

    await webviewPage.keyboard.type(
      "，找出可能遗漏的边界情况，并给出修改建议。同时保留现有接口行为，不要改动公共类型。请帮我检查当前文件中的权限处理逻辑，找出可能遗漏的边界情况，并给出修改建议。",
    );
    await elementScreenshotWebp(
      webviewPage.locator(".input-container"),
    ...
    await screenshotWebp(
      webviewPage,
      "../../docs/public/screenshots/spec-basic-chat.webp",
    );
    ...
    await injector.simulateExtensionMessage("slashCommandsResponse", {
      commands: [
        // SDK 内置技能命令
        {
          id: "code-review",
          name: "code-review",
          description: "审查当前代码变更，检测正确性缺陷并提供优化建议",
        },
        {
          id: "deep-research",
          name: "deep-research",
        ...
        },
        {
          id: "init",
          name: "init",
          description: "分析代码库并生成 AGENTS.md 文件，指导后续 Agent 工作",
        },
        {
          id: "loop",
          name: "loop",
          description: "按固定间隔循环执行指令（如 /loop 5m /code-review）",
        },
        {
          id: "settings",
          name: "settings",
          description: "管理 Wave 设置：hooks、环境变量、权限、MCP、内存等",
        },
        {
          id: "simplify",
          name: "simplify",
    ...
    await screenshotWebp(
      webviewPage,
      "../../docs/public/screenshots/spec-slash-commands.webp",
    );
    ...
    await injector.simulateExtensionMessage("fileSuggestionsResponse", {
      ...
      suggestions: [
        ...
        },
        {
          ...
          relativePath: "src/services/payment/PaymentService.ts",
    ...
    await webviewPage.waitForSelector(".suggestion-item", {
      state: "visible",
      timeout: 5000,
    });
    await screenshotWebp(
      webviewPage,
      "../../docs/public/screenshots/spec-file-suggestions.webp",
    );
    await webviewPage.keyboard.press("Escape");
    await webviewPage.keyboard.press("Control+A");
    await webviewPage.keyboard.press("Backspace");
