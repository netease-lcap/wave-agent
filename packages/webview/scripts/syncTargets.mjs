...
const distFiles = fs
  .readdirSync(distDir)
  .filter((file) => !/^terminal\./.test(file));
for (const file of distFiles) {
  fs.copyFileSync(path.join(distDir, file), path.join(vsceDest, file));
}
console.log(
  `[sync-targets] vsce: copied ${distFiles.length} files (${distFiles.join(", ")})`,
);
...
fs.copyFileSync(
  path.join(themeDir, "theme-base-dark.css"),
  path.join(jbDest, "theme-base.css"),
);
fs.copyFileSync(
  path.join(themeDir, "theme-base-light.css"),
  path.join(jbDest, "theme-base-light.css"),
);
console.log(
  "[sync-targets] jetbrains: copied chat.js, chat.css, theme-base.css, theme-base-light.css",
);
