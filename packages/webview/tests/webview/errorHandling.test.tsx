...
describe("Error Message Display", () => {
  ...
  it("should display error messages from agent", async () => {
    ...
    const errorMessage = MockDataGenerator.createErrorMessage(
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Failed to read file: Permission denied"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("should allow recovery after error", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Sure! Let me try that again."),
      ).toBeInTheDocument();
    });
  });
  ...
  it("should handle errors during streaming", async () => {
        ...
        messages: [
          {
            ...
            blocks: [
              {
                type: "text",
                content: "I was working on your request when...",
              },
            ],
          },
        ],
        ...
        messages: [
          {
            ...
            timestamp: "2024-01-01T00:00:00.000Z",
            blocks: [
              { type: "error", content: "Connection lost during streaming" },
            ],
          },
        ],
      });
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Connection lost during streaming"),
      ).toBeInTheDocument();
    });
