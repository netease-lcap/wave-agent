...
describe("Session Management", () => {
  it("should load and display sessions in the history popup", () => {
    ...
    expect(
      screen.getByTestId("session-list-item-session-1"),
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("session-list-item-session-2"),
    ).toBeInTheDocument();
    expect(screen.getByTestId("session-list-item-session-1")).toHaveTextContent(
      "First session hello",
    );
    expect(screen.getByTestId("session-list-item-session-2")).toHaveTextContent(
  ...
  it("should filter sessions and highlight matches in the search box", () => {
    ...
    expect(
      screen.queryByTestId("session-list-item-session-2"),
  ...
  it("should show empty state when no session matches the query", () => {
    ...
    act(() => {
      fireEvent.change(searchInput, {
        target: { value: "nonexistent-keyword" },
      });
    });
  ...
  it("should create new session after clear chat through callbacks", async () => {
    ...
    const conversation = [
      ...
      MockDataGenerator.createAssistantMessage(
        "Hi! This is in the original session.",
      ),
    ];
    act(() => {
    ...
    openSessionListPopup();
    expect(
      screen.getByTestId("session-list-item-session-original"),
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("session-list-item-session-new"),
    ).toBeInTheDocument();
  });
  ...
  it("should show 新对话 title when current session is not yet in the list", () => {
    ...
    openSessionListPopup();
    expect(
      screen.getByTestId("session-list-item-session-in-list"),
    ).toBeInTheDocument();
  });
  ...
  it("should derive the title from the first user message once one is sent", () => {
    ...
    act(() => {
      sendHostMessage(
        fixtures.updateMessages([MockDataGenerator.createUserMessage("hi")]),
      );
    });
...
describe("input drafts per session (desktop-app.md 会话管理 scenario 11/12)", () => {
  ...
  it("switching sessions resets the input even when both drafts are equal (empty)", async () => {
    ...
    act(() => {
      sendHostMessage(
        fixtures.setInitialState({ session: sessionA, messages: [] }),
      );
    });
    const input = screen.getByTestId("message-input");
    ...
    act(() => {
      sendHostMessage(
        fixtures.setInitialState({ session: sessionB, messages: [] }),
      );
    });
