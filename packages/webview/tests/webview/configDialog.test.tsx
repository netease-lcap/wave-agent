...
function renderDialog(overrides?: {
  ...
  const result = render(
    <ConfigDialog
      configurationData={
        overrides?.configurationData ?? { language: "Chinese" }
      }
...
describe("ConfigDialog", () => {
  ...
  it("switches to the model tab and renders model fields", () => {
    ...
    expect(
      screen
        .getByRole("tab", { name: "直连设置" })
        .getAttribute("aria-selected"),
    ).toBe("true");
    expect(
      screen.getByText("保存后，优先使用此配置，无需登录即可正常使用插件。"),
    ).toBeTruthy();
  ...
  it("uses the correct placeholder text for Base URL", () => {
    ...
    expect(screen.getByLabelText("Base URL").getAttribute("placeholder")).toBe(
      "请输入 Base URL",
    );
  ...
  it("reflects the SDD toggle checked state from projectSettings", () => {
    ...
    fireEvent.click(screen.getByRole("tab", { name: "项目设置" }));
    expect((screen.getByTestId("sdd-toggle") as HTMLInputElement).checked).toBe(
      true,
    );
  });
  ...
  it("disables the SDD toggle while projectSettings is still loading", () => {
    ...
    fireEvent.click(screen.getByRole("tab", { name: "项目设置" }));
    expect(
      (screen.getByTestId("sdd-toggle") as HTMLInputElement).disabled,
    ).toBe(true);
  });
});
