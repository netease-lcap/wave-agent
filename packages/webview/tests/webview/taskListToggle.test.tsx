...
describe("task list capped at 4 visible rows", () => {
  it("renders all tasks in the DOM, capping the visible area so the rest scroll", async () => {
    ...
    expect(document.querySelectorAll(".task-list-items .task-row").length).toBe(
      6,
    );
...
describe("collapse via updateTasks", () => {
  it("collapses when isTaskListCollapsed:true", async () => {
    ...
    expect(
      document
        .querySelector(".task-list-chevron")
...
describe("scroll updated task into view", () => {
  it("scrolls the status-changed task to center on updateTasks", async () => {
    ...
    // 滚动的是 #3 这一行
    const scrolledEl =
      (scrollIntoView.mock.instances[0] as Element) ||
      document.querySelector('[data-task-id="3"]');
