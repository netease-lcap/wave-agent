...
describe('ConfirmDialog', () => {
    it('renders title, description and both buttons', () => {
        ...
        expect(screen.getByText('这将删除之后的所有消息并撤销相关的文件更改。')).toBeInTheDocument();
        expect(screen.getByTestId('confirm-dialog-cancel')).toHaveTextContent('取消');
        expect(screen.getByTestId('confirm-dialog-confirm')).toHaveTextContent('确定');
    });
    ...
    it('supports custom button text', () => {
        ...
        expect(screen.getByTestId('confirm-dialog-confirm')).toHaveTextContent('删除');
        expect(screen.getByTestId('confirm-dialog-cancel')).toHaveTextContent('再想想');
    });

    it('calls onConfirm when the confirm button is clicked', () => {
        const onConfirm = vi.fn();
        render(<ConfirmDialog title="t" onConfirm={onConfirm} onCancel={vi.fn()} />);
