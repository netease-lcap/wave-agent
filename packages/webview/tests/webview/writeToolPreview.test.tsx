...
describe("WriteToolPreview", () => {
  it("renders path, stats and content preview for a valid Write block", () => {
    ...
    const { container } = render(
      <WriteToolPreview toolBlock={block} vscode={vscode} />,
    );

    expect(container.querySelector(".write-tool-path")).toHaveTextContent(
      "/a/b.md",
    );
    expect(container.querySelector(".write-tool-stats")).toHaveTextContent(
      "File created (3 lines, 8 characters)",
    );
  ...
  it("renders only the header without crashing on invalid JSON parameters", () => {
    ...
    expect(
      container.querySelector(".write-preview-box"),
    ).not.toBeInTheDocument();
    expect(
      container.querySelector(".write-tool-stats"),
    ).not.toBeInTheDocument();
  });
});
