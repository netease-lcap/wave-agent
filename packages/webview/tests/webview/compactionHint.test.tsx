...
import { renderChatApp, screen, sendCommand, createMockVscode } from './test-utils';
...
describe('Compaction hint', () => {
    ...
    it('shows the hint after the blinking cursor while compacting and hides it on completion', () => {
        ...
        expect(screen.getByTestId('messages-container').className).toContain('compacting');

        sendCommand('compactionStateChange', { isCompacting: false });

        expect(screen.queryByTestId('compaction-hint')).not.toBeInTheDocument();
        expect(screen.getByTestId('messages-container').className).not.toContain('compacting');
    });
    ...
    it('ignores compaction state tagged for a different pane (desktop split-view)', () => {
        ...
        act(() => {
            window.dispatchEvent(new MessageEvent('message', {
                data: { command: 'compactionStateChange', isCompacting: true, paneId: 'pane-b' }
            }));
        });
        expect(screen.queryByTestId('compaction-hint')).not.toBeInTheDocument();
        ...
        act(() => {
            window.dispatchEvent(new MessageEvent('message', {
                data: { command: 'compactionStateChange', isCompacting: true, paneId: 'pane-a' }
            }));
        });
