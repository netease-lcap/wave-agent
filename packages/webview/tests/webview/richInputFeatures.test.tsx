...
describe("Rich Input Features", () => {
  ...
  it("should handle mixed text and multiple context tags correctly", async () => {
    ...
    const req1 = calls1.find(
      (m: { command: string }) => m.command === "requestFileSuggestions",
    );
    ...
    const sentMarkdown = ((sendMsg.text as string) || "")
      .replace(/\u00A0/g, " ")
      .trim();
    expect(sentMarkdown).toBe(
      "Check these files: [@file:file1.ts] and also [@file:file2.ts]",
    );
  });
  ...
  it("should insert inline selection tag and render it in history", async () => {
    ...
    expect(sentMessage.text).toContain(
      "[Selection: /workspace/src/app.ts|app.ts#1-10]",
    );
