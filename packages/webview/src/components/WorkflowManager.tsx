      ...
      <div className="mcp-server-list">
        <div className="mcp-server-item">
          <div className="mcp-server-info">
            <div className="mcp-server-header">
              ...
              <span style={{ color: statusColor(selectedRun.status), fontSize: '12px' }}>
                {selectedRun.status}
              </span>
            </div>
            <div style={{ fontSize: '12px', color: 'var(--vscode-descriptionForeground)' }}>
              <span style={{ color: '#569cd6' }}>Run ID:</span> {selectedRun.runId}
            </div>
            {selectedRun.meta.description && (
              <div style={{ fontSize: '12px', color: 'var(--vscode-descriptionForeground)' }}>
                <span style={{ color: '#569cd6' }}>Description:</span> {selectedRun.meta.description}
              </div>
            )}
            <div style={{ fontSize: '12px', color: 'var(--vscode-descriptionForeground)' }}>
              <span style={{ color: '#569cd6' }}>Started:</span> {formatTime(selectedRun.startTime)}
              {selectedRun.endTime && (
                <>
                  {' | '}
                  <span style={{ color: '#569cd6' }}>Ended:</span> {formatTime(selectedRun.endTime)}
                </>
              )}
              {' | '}
              <span style={{ color: '#569cd6' }}>Duration:</span> {formatDuration(elapsed)}
            </div>
            <div style={{ fontSize: '12px', color: 'var(--vscode-descriptionForeground)' }}>
              <span style={{ color: '#569cd6' }}>Agents:</span> {selectedRun.totalAgents}
              {' | '}
              <span style={{ color: '#569cd6' }}>Tokens:</span> {formatTokens(selectedRun.totalTokens)}
            </div>
            <div style={{ fontSize: '12px', color: 'var(--vscode-descriptionForeground)', fontFamily: 'var(--vscode-editor-font-family, monospace)', wordBreak: 'break-all' }}>
              <span style={{ color: '#569cd6' }}>Script:</span> {selectedRun.scriptPath}
            </div>
            {selectedRun.error && (
              <div style={{ fontSize: '12px', color: '#f48771', wordBreak: 'break-all' }}>
                <span style={{ color: '#569cd6' }}>Error:</span> {selectedRun.error}
              </div>
            )}
          </div>
        ...
        {selectedRun.phases.length > 0 && (
          <div style={{ marginTop: '12px' }}>
            <div style={{ fontSize: '12px', color: '#4ec9b0', marginBottom: '4px' }}>Phases:</div>
            {selectedRun.phases.map((phase, i) => (
              <div key={i} style={{ fontSize: '12px', color: 'var(--vscode-descriptionForeground)', marginLeft: '8px', marginBottom: '2px' }}>
                <span style={{ color: '#569cd6' }}>{phase.title}</span>
                {' — '}
                {phase.agentCount} agents | {formatTokens(phase.tokens)} tokens | {formatDuration(phase.elapsed)}
              </div>
            ))}
          </div>
        )}
          ...
          >
            <div className="mcp-server-info">
              <div className="mcp-server-header">
                ...
                <span className="mcp-server-name">[{run.runId.slice(0, 8)}] {run.meta.name}</span>
                <span style={{ color: statusColor(run.status), fontSize: '12px' }}>{run.status}</span>
              </div>
              {run.meta.description && (
                <div style={{ fontSize: '12px', color: 'var(--vscode-descriptionForeground)' }}>{run.meta.description}</div>
              )}
              <div style={{ fontSize: '12px', color: 'var(--vscode-descriptionForeground)' }}>
                {run.totalAgents} agents · {formatTokens(run.totalTokens)} tokens · {formatDuration(elapsed)}
              </div>
              {phaseText && (
                <div style={{ fontSize: '12px', color: 'var(--vscode-descriptionForeground)' }}>
                  {run.phases.map((p, pi) => (
            ...
            <div className="mcp-server-actions">
              {run.status === 'running' && (
                <button
                  className="mcp-disconnect-btn"
                  onClick={(e) => { e.stopPropagation(); handleStop(run.runId); }}
      ...
      <div ref={dialogRef} className="configuration-dialog" style={{ minWidth: '560px', maxWidth: '760px' }}>
        ...
        <div className="mcp-container">
          ...
          <div className="configuration-actions">
            ...
            <button type="button" onClick={onCancel} className="configuration-cancel-btn">
              关闭
            </button>
          </div>
        </div>
