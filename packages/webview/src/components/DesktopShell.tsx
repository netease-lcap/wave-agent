...
interface DesktopShellProps {
  ...
  /** Batch 2 session board: rendered over the pane rows when open. */
  sessionBoardOpen?: boolean;
  onCloseSessionBoard?: () => void;
  ...
  /** 插件市场整页（spec ecosystem/plugin.md「插件市场」场景 1/2）：覆盖 pane 行，
   *  侧边栏保留。 */
  pluginMarketOpen?: boolean;
  onClosePluginMarket?: () => void;
  pluginMarket?: React.ReactNode;
  /** 侧边栏「插件市场」入口高亮（spec 场景 1/2）。 */
  pluginMarketActive?: boolean;
  onTogglePluginMarket?: () => void;
  /** 打开（不切换）插件市场整页——透传给 pane 实例供 `/plugin` 委派（spec
   *  ecosystem/plugin.md 场景 4）。 */
...
export const DesktopShell: React.FC<DesktopShellProps> = ({
  ...
  const [rowResizePreview, setRowResizePreview] = useState<number[] | null>(
    null,
  );
  ...
  const resizePreviewRef = useRef<{ row: number; widths: number[] } | null>(
    null,
  );
  ...
  const canAddPane = useCallback(
    (row: number): boolean => {
      const rowCount = panes.filter(
        (p) => (p.row === 1 ? 1 : 0) === row,
      ).length;
      const rowWidth =
        rowRefs.current[row]?.getBoundingClientRect().width ??
        window.innerWidth;
  ...
  const canSplitRows = useCallback((): boolean => {
    const height =
      rowsContainerRef.current?.getBoundingClientRect().height ??
      window.innerHeight;
    return height / 2 >= MIN_ROW_HEIGHT;
  }, []);

  const rowOfPane = useCallback(
    (paneId: string): 0 | 1 => {
    ...
    (workdir: string, sessionId: string, opts?: OpenPaneOptions) => {
      if (!panes.some((p) => p.sessionId === sessionId)) {
        if (opts?.newRow && !hasTwoRows) {
          ...
          opts?.row === undefined &&
          opts?.newRow === undefined &&
          !canAddPane(rowOfPane(focusedPaneId ?? ""))
        ) {
  ...
  useLayoutEffect(() => {
    ...
    panes.forEach((pane) => {
      const header = paneNodes.current
        .get(pane.paneId)
        ?.querySelector<HTMLElement>(".chat-header");
      if (!header) return;
      ...
      const onDragStart = (e: DragEvent) => {
        ...
        e.dataTransfer.setData(
          PANE_DRAG_MIME,
          JSON.stringify({ paneId: pane.paneId }),
        );
        try {
  ...
  const showDropIndicator = useCallback(
    (row: number, boundary: number, markerX: number) => {
  ...
  // is impossible.
  const updateEdgeZone = useCallback(
    (e: React.DragEvent, enabled: boolean): DropZone | null => {
      ...
      const zone: DropZone | null =
        e.clientY < rect.top + band
          ? "above"
          : e.clientY > rect.bottom - band
            ? "below"
            : null;
      if (!zone) {
        setDropZone(null);
  ...
  const handlePaneDragOver = useCallback(
    (e: React.DragEvent, row: number, index: number) => {
  ...
  const handleSeparatorDragOver = useCallback(
    (e: React.DragEvent, row: number, boundary: number) => {
      if (!e.dataTransfer.types.includes(PANE_DRAG_MIME)) return;
      e.preventDefault();
      try {
        e.dataTransfer.dropEffect = "move";
  ...
  const handleSessionDragOver = useCallback(
    (e: React.DragEvent, row: number) => {
      ...
      const lastEl = paneNodes.current.get(
        rowPanes[rowPanes.length - 1]?.paneId,
      );
      if (lastEl)
        showDropIndicator(
          row,
    ...
    (e: React.DragEvent, row: number) => {
      ...
      try {
        ...
        handleOpenPane(workdir, sessionId, {
          row: row === 1 ? 1 : 0,
          insertionIndex:
            boundary && boundary.row === row ? boundary.index : undefined,
        });
      } catch {
  ...
  const handleSeparatorMouseDown = useCallback(
    (e: React.MouseEvent, row: number, separatorIndex: number) => {
      e.preventDefault();
      const rowPanes = panes.filter((p) => (p.row === 1 ? 1 : 0) === row);
      const startX = e.clientX;
      const widths = rowPanes.map(
        (pane) =>
          paneNodes.current.get(pane.paneId)?.getBoundingClientRect().width ??
          0,
      );
      ...
      const onMouseMove = (ev: MouseEvent) => {
        ...
        const clamped = Math.max(
          ...
          Math.min(
            ...
            widths[separatorIndex] + ev.clientX - startX,
          ),
        );
        const next = [...widths];
      ...
      const onMouseUp = () => {
        ...
        vscode.postMessage({
          ...
          row,
          widths: preview.widths.map((w) => w / sum),
        });
      };
  ...
  const handleRowSeparatorMouseDown = useCallback(
    (e: React.MouseEvent) => {
      e.preventDefault();
      ...
      const onMouseMove = (ev: MouseEvent) => {
        const top = Math.max(
          MIN_ROW_HEIGHT,
          Math.min(total - MIN_ROW_HEIGHT, startTop + ev.clientY - startY),
        );
  ...
  const focusedSessionId = panes.find(
    (p) => p.paneId === focusedPaneId,
  )?.sessionId;
  ...
  const focusedHost =
    panes.find((p) => p.paneId === focusedPaneId)?.host ?? host.host;
  ...
  // 侧边栏里任何「换对话」的动作都会换掉插件市场的锚点（spec ecosystem/plugin
  // A-018 / 场景 2）：选中另一条对话、Cmd 点击在新分屏打开、新建对话——都先退出
  // 整页再动作。用户点完看到的是对话本身；插件视图也不会留在锚点已经变了的旧
  // 列表上（列表快照与写操作必须同一个锚点）。
  const closePluginMarketThen = (action: () => void) => {
    onClosePluginMarket?.();
    action();
  };
  ...
  return (
    <div className="desktop-layout" data-testid="desktop-shell">
      <DesktopSidebar
        onNewSession={() =>
          closePluginMarketThen(() =>
            vscode.postMessage({ command: "newSession" }),
          )
        }
        onNewSessionInPane={() =>
          closePluginMarketThen(() =>
            vscode.postMessage({ command: "desktopNewSessionInPane" }),
          )
        }
        isStreaming={false}
        ...
        onSelectSession={(workdir, sessionId) =>
          closePluginMarketThen(() => host.onSelectSession(workdir, sessionId))
        }
        onOpenPane={(workdir, sessionId) =>
        ...
        pluginMarketActive={pluginMarketActive}
        onTogglePluginMarket={onTogglePluginMarket}
      />
      <div
                        ...
                        <React.Fragment key={pane.paneId}>
                          ...
                          >
                            <ChatApp
                              ...
                              onOpenPluginMarketFromPane={onOpenPluginMarket}
                              prefillRequest={
