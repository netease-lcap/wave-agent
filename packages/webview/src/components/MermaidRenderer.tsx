...
const FullscreenModal: React.FC<FullscreenModalProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="fullscreen-help">滚轮缩放 | 拖拽移动 | ESC关闭</div>
    </div>
  );
};
...
export const MermaidRenderer: React.FC<MermaidRendererProps> = ({
  ...
  const renderMermaid = useCallback(async () => {
    ...
    try {
      ...
      if (
        !diagramContent.match(
          /^(graph|flowchart|sequenceDiagram|classDiagram|stateDiagram|stateDiagram-v2|gantt|pie|gitGraph|journey|quadrantChart|requirementDiagram|erDiagram|mindmap|timeline|sankey|block-beta|xychart-beta|packet-beta|architecture-beta|c4Context|c4Container|c4Component|c4Dynamic|c4Deployment)/,
        )
      ) {
        throw new Error(
          "Invalid diagram type. Please start with a valid Mermaid diagram declaration.",
        );
      }
      ...
      try {
        ...
        if (
          ...
          setError(
            `Syntax error: ${errorMessage}. Please check your diagram syntax.`,
          );
        } else if (
          errorMessage.includes("firstChild") ||
          errorMessage.includes("null")
        ) {
          setError(
            "Rendering error: DOM element not available. Please try again.",
          );
        } else {
  ...
  useEffect(() => {
    if (activeTab === "preview" && cleanedContent.current.trim()) {
      ...
      const contentChanged =
        cleanedContent.current !== lastRenderedContent.current;
