 ...
 */
export async function screenshotWebp(page: Page, filePath: string, options: WebpScreenshotOptions = {}): Promise<void> {
 ...
 */
export async function elementScreenshotWebp(locator: Locator, filePath: string): Promise<void> {
    await locator.scrollIntoViewIfNeeded();
    const box = await locator.boundingBox();
    if (!box) {
        throw new Error(`Element screenshot failed: element not visible for ${filePath}`);
    }
