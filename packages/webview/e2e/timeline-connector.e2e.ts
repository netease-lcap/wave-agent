...
test.describe("Timeline connector geometry demo", () => {
  test("continuous line through a multi-assistant group; none for single group", async ({
    webviewPage,
  }) => {
    ...
    const geo = await webviewPage.evaluate(() => {
      ...
      const rows = Array.from(
        multi.querySelectorAll(".timeline-row"),
      ) as HTMLElement[];
      ...
      return {
        ...
        firstDotCenter,
        lastDotCenter,
        singleIsSingleClass: single.classList.contains(
          "assistant-group--single",
        ),
    ...
    expect(
      Math.abs(geo.segments[0].segEnd - geo.segments[1].segStart),
    ).toBeLessThanOrEqual(1);

    // First segment starts at the first dot center (dot top 15 + radius 3 = 18px).
    expect(
      Math.abs(geo.segments[0].segStart - geo.firstDotCenter),
    ).toBeLessThanOrEqual(1);
    ...
    expect(
      Math.abs(
        geo.segments[geo.segments.length - 1].segEnd - geo.lastDotCenter,
      ),
    ).toBeLessThanOrEqual(1);
