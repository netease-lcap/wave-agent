...
test.describe('Built-in SDD Plugin Demo', () => {
    test('should show SDD builtin plugin toggle in project settings tab', async ({ webviewPage }) => {
        ...
        await expect(webviewPage.getByText('规格驱动开发（SDD）插件')).toBeVisible();

        await elementScreenshotWebp(dialog, '../../docs/public/screenshots/spec-sdd-plugin.webp');
    });
});
