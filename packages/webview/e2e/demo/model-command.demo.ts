...
test.describe('Product Spec: /model 命令', () => {
    test('should capture model popup screenshot', async ({ webviewPage }) => {
        ...
        await webviewPage.waitForSelector('.model-popup-item', { state: 'visible', timeout: 5000 });
        ...
        await screenshotWebp(webviewPage, '../../docs/public/screenshots/spec-model-popup.webp');
    });

    test('should switch model with keyboard navigation', async ({ webviewPage }) => {
        const injector = new MessageInjector(webviewPage);
