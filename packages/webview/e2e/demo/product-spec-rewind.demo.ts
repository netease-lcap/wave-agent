...
test.describe('Product Spec: Rewind', () => {
    ...
    test('should capture /rewind popup screenshot', async ({ webviewPage }) => {
        ...
        const messages = [
            ...
            MockDataGenerator.createAssistantMessage('我已经分析了 PaymentService 的代码，发现 `processPayment` 方法中存在竞态条件。当前的悲观锁实现会导致高并发下性能下降，建议改用乐观锁...'),
            MockDataGenerator.createUserMessage('好的，请为乐观锁实现编写单元测试，覆盖并发冲突场景'),
            MockDataGenerator.createAssistantMessage('已为乐观锁实现编写了 5 个单元测试，覆盖了并发冲突、重试机制和超时场景...'),
            MockDataGenerator.createUserMessage('测试全部通过了，现在帮我把改动提交到 git')
        ];
        await injector.updateMessages(messages);
        await injector.endStreaming();
        ...
        await injector.simulateExtensionMessage('rewindCheckpoints', {
            checkpoints: [
                { id: 'cp-1', content: '帮我分析 PaymentService 的并发问题，看看有没有竞态条件' },
                { id: 'cp-2', content: '好的，请为乐观锁实现编写单元测试，覆盖并发冲突场景' },
                { id: 'cp-3', content: '测试全部通过了，现在帮我把改动提交到 git' }
            ]
        });

        // Wait for the popup to appear with items
        await webviewPage.waitForSelector('.rewind-popup-item', { state: 'visible', timeout: 5000 });
