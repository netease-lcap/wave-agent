import { test as webviewTest, expect } from '../utils/webviewTestHarness.js';
import { test as desktopTest } from '../utils/desktopTestHarness.js';
...
desktopTest.describe('Product Spec: Action Confirm Dialog (Delete Session)', () => {
    desktopTest('should capture the delete-session confirmation dialog', async ({ webviewPage }) => {
        const injector = new MessageInjector(webviewPage);
            ...
            groups: [
                {
                    workdir: '/Users/dev/projects/wave-agent',
                    sessions: [
                        { sessionId: 'sess-a1', title: '帮我修复登录页的样式问题', lastActiveAt: new Date('2026-07-27T10:12:00Z').getTime(), hasWorktree: true },
                        { sessionId: 'sess-a2', title: '给 bashTool 增加超时参数', lastActiveAt: new Date('2026-07-26T09:40:00Z').getTime(), hasWorktree: false }
                    ]
                }
            ]
        });
