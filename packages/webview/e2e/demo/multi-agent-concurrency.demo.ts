...
test.describe('Product Specification Screenshots - Multi Agent Concurrency', () => {
    test('capture multi-agent concurrency features', async ({ webviewPage }) => {
                ...
                blocks: [
                    {
                        ...
                        parameters: JSON.stringify({ subagent_type: 'Explore', description: '梳理支付模块的代码结构', prompt: '...' }),
                    ...
                    },
                    {
                        ...
                        parameters: JSON.stringify({ subagent_type: 'general-purpose', description: '审查分布式事务中的竞态条件', prompt: '...' }),
                    ...
                    },
                    {
                        ...
                        parameters: JSON.stringify({ subagent_type: 'plan', description: '盘点测试覆盖缺口', prompt: '...' }),
        ...
        await elementScreenshotWebp(webviewPage.locator('.messages-container'), '../../docs/public/screenshots/spec-subagent-concurrency.webp');
                ...
                blocks: [
                    {
                        ...
                        result: 'Created worktree at /home/dev/projects/nebula-platform/.wave/worktrees/pay-refactor on branch worktree-pay-refactor. The session is now working in the worktree. Use ExitWorktree to leave mid-session, or exit the session to be prompted.'
                    }
                ]
            }
        ];
        ...
        await webviewPage.waitForSelector('.tool-container');
        await elementScreenshotWebp(webviewPage.locator('.messages-container'), '../../docs/public/screenshots/spec-worktree-enter.webp');
                ...
                blocks: [
                    {
                        ...
                        parameters: JSON.stringify({ subagent_type: 'general-purpose', description: '调研支付网关的兼容性', prompt: '...', run_in_background: true }),
                        shortResult: 'Agent started in background: task-agent-003 → /tmp/task-agent-003.log'
                    },
                    {
                        type: 'tool',
                        name: AGENT_TOOL_NAME,
                        stage: 'end',
                        success: true,
                        compactParams: 'general-purpose: 审查提现流程的边界条件（后台运行）',
                        parameters: JSON.stringify({ subagent_type: 'general-purpose', description: '审查提现流程的边界条件', prompt: '...', run_in_background: true }),
                        shortResult: 'Agent started in background: task-agent-004 → /tmp/task-agent-004.log'
                    }
                ]
            },
            {
                ...
                blocks: [
                    {
                        ...
                        taskType: 'agent',
                        status: 'completed',
                        summary: '后台调研完成：支付网关兼容性分析报告已生成，支持 Stripe / PayPal / 支付宝三种网关，建议优先接入 Stripe。'
                    },
                    {
                        type: 'task_notification',
                        ...
                        taskType: 'agent',
                        status: 'completed',
                        summary: '后台审查完成：提现流程在金额为 0、余额不足、重复提交三类边界条件下存在缺陷，已生成修复建议。'
                    }
                ]
            }
        ];
        ...
        await elementScreenshotWebp(webviewPage.locator('.messages-container'), '../../docs/public/screenshots/spec-background-subagent.webp');
    });
});
