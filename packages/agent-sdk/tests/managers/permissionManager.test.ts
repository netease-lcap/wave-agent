...
describe("PermissionManager", () => {
  ...
  describe("checkPermission method", () => {
    describe("Bash Write Redirection Interception", () => {
      it("should deny bash commands with write redirections and provide a reminder", async () => {
        ...
        expect(result.message).toContain("Bash-based file writing operations");
        expect(result.message).toContain(
          "Please use the dedicated 'Write' or 'Edit' tools",
        );
      });

      it("should deny bash commands with append redirections", async () => {
        const context: ToolPermissionContext = {
          toolName: "Bash",
          permissionMode: "default",
          toolInput: { command: "echo 'test' >> test.txt" },
      ...
      it("should deny complex bash commands with heredoc and redirection", async () => {
        const context: ToolPermissionContext = {
          toolName: "Bash",
          permissionMode: "default",
          toolInput: {
            command: "cat <<EOF > demo.py\nprint('hello')\nEOF",
          },
        };
      ...
      it("should allow bash commands with stream redirections (e.g., 2>&1)", async () => {
        ...
        // Note: 2>&1 is now ignored by isBashWriteRedirect
        const result = await permissionManager.checkPermission(context);
        expect(result.behavior).toBe("deny"); // Still denied because it's a restricted tool without callback
        expect(result.message).toContain("requires permission approval");
      });

      it("should deny bash write redirections even in bypassPermissions mode", async () => {
        const context: ToolPermissionContext = {
          toolName: "Bash",
