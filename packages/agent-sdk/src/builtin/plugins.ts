...
const guidance = [
  ...
  "- 阶段衔接一律用 AskUserQuestion 单选让用户点击决策，不要求自然语言：spec 确认（确认通过/需要修改）、是否制定技术方案（plan）均通过单选推进。",
  "- 可选 plan 阶段：进入 plan 模式制定技术方案，批准后再编码；也可跳过——最短流程=规格+编码，最长=规格+plan+编码。",
  "- 用 task 工具追踪进度：规格、plan、编码各阶段开始前用 TaskCreate 创建任务并标记进行中（TaskUpdate），完成/批准/确认后标记完成，让用户在任务列表中看到当前所处阶段。",
].join("\\n");
...
name: specify
description: 根据自然语言描述创建或更新功能规格说明，并通过单选衔接可选技术方案（plan）与编码阶段。
...
0. **创建进度任务**：用 TaskCreate 创建「编写功能规格」任务，并用 TaskUpdate 标记进行中。后续每个阶段（plan、编码）同样在开始前创建任务、结束后更新状态，让用户在任务列表中看到当前进度。
