...
const guidance = [
  ...
  "- 规格确认与阶段衔接一次完成：通过 AskUserQuestion 单选让用户点击决策（选项：直接实现 / 制定技术方案 / 其他——选「其他」=规格需调整，按反馈修改后再次询问）。",
...
5. **校验并确认规格**：
   ...
   - 输出规格文件路径，并通过 AskUserQuestion 单选请求决策（选项：直接实现 / 制定技术方案 / 其他）
   - 选「其他」→ 视为规格需要调整：按用户反馈更新规格后重新校验，并再次单选
