...
async function runExitCodeCommunicationExample() {
  ...
  writeFileSync(successHook, `#!/bin/bash
...
`, { mode: 0o755 });
  ...
  writeFileSync(blockingHook, `#!/bin/bash
...
`, { mode: 0o755 });
  ...
  writeFileSync(warningHook, `#!/bin/bash
  ...
  try {
    ...
    const agent = await Agent.create({
      ...
      callbacks: {
        ...
        onHookMessageAdded: (hookEvent: string, content: string, metadata?: Record<string, unknown>) => {
    ...
    try {
      ...
      await agent.sendMessage("Please use the Bash tool to list files in the current directory.");
    ...
    console.log("• Warning Hook (exit 1): Tool executes with warning displayed");
...
async function runInteractiveExitCodeTest() {
  ...
  console.log("This function demonstrates how to create and test hooks with different exit codes.\n");
...
exit 42`
  ...
  console.log("• Exit 1 or other: Show warning but continue (non-blocking error)");
}
...
async function testHookOutputParsing() {
  ...
  testCases.forEach(testCase => {
    ...
    console.log(`Error Messages: ${parsed.errorMessages.length > 0 ? parsed.errorMessages.join(', ') : 'None'}`);
  });
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  runExitCodeCommunicationExample()
    ...
    .then(() => {
      ...
      console.log("This demonstrates User Story 1: Simple Exit Code Communication");
