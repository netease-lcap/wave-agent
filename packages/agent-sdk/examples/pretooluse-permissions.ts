  ...
  askPermission: {
    ...
    stdout: JSON.stringify({
      ...
      permissionDecisionReason:
        "This tool will read sensitive configuration files. Do you want to proceed?",
      updatedInput: {
  ...
  try {
    const result = await hookExecutor.executePreToolUseWithPermissions(
      ...
      (request) => {
        ...
        console.log(
          `   Tool Input: ${JSON.stringify(request.toolInput, null, 2)}`,
        );
      },
    );
...
async function example2_AutoAllow() {
  ...
  try {
    ...
    console.log(`   Requires Permission: ${result.requiresUserPermission}`);
    console.log(
      `   Updated Input: ${JSON.stringify(result.updatedInput, null, 2)}`,
    );
  } catch (error) {
...
async function example4_AgentIntegration() {
  ...
  try {
    ...
    console.log(
      `   Pending Permissions: ${agent.getPendingPermissions().length}`,
    );
...
async function example5_InteractiveSimulator() {
  ...
  for (const [index, scenario] of scenarios.entries()) {
    ...
    const decision = userDecisions[index % userDecisions.length] as
      | "allow"
      | "deny";
    ...
    console.log(
      `   ✅ Permission ${decision === "allow" ? "granted" : "denied"}`,
    );
    console.log();
...
async function main() {
  ...
  try {
    ...
    hookExecutor.executeHookWithOutput = async (
      command,
      context,
      hookEvent,
    ) => {
      ...
      if (command.includes("security-hook")) {
        ...
        return originalExecuteHookWithOutput.call(
          hookExecutor,
          command,
          context,
          hookEvent,
      ...
      return {
        ...
        parsedOutput: (
          await import("../src/utils/hookOutputParser.js")
        ).parseHookOutput({
          exitCode: mockResult.exitCode,
          stdout: mockResult.stdout,
          stderr: mockResult.stderr,
    ...
    console.log(
      "🎉 All PreToolUse Permission examples completed successfully!",
    );
    console.log("\n📖 Summary:");
...
async function simulateUserInput(
  prompt: string,
  defaultResponse: "allow" | "deny" = "allow",
): Promise<"allow" | "deny"> {
  console.log(`❓ ${prompt}`);
