...
import { readClipboardImage, cleanupTempImage, hasClipboardImage } from '../src/utils/clipboard';
import { createTestImage } from './utils/testImageHelper';
...
describe('Clipboard Image Utils', () => {
  ...
  describe('readClipboardImage', () => {
    ...
    it('should handle empty clipboard', async () => {
      ...
      if (!isXclipAvailable) {
        logger.info('Skipping empty clipboard test because xclip is not available');
        return;
      }
    ...
    it('should read image from clipboard when image is present', async () => {
      ...
      try {
        // 根据平台将测试图片复制到剪贴板
        if (platform === 'darwin') {
          await execAsync(`osascript -e 'set the clipboard to (read (POSIX file "${testImagePath}") as JPEG picture)'`);
        } else if (platform === 'win32') {
          const script = `
          ...
          await execAsync(`xclip -selection clipboard -t image/png -i "${testImagePath}"`);
        }
  ...
  describe('hasClipboardImage', () => {
    ...
    it('should return true when image is in clipboard', async () => {
      ...
      try {
        ...
        logger.warn('Failed to set up test clipboard image for hasClipboardImage test:', error);
      }
    });
  });
