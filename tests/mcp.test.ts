...
import { initializeMCPClient, MCPClient } from '../src/utils/mcp';
import fs from 'fs';
...
describe('MCP Client', () => {
  ...
  describe('initializeMCPClient', () => {
    ...
    it('should return null when mcp.json parsing fails', async () => {
      ...
      expect(result).toBeNull();
      expect(mockReadFile).toHaveBeenCalledWith('/test/workdir/mcp.json', 'utf8');
    });
  ...
  describe('MCPClient', () => {
    ...
    it('should throw error when calling tool with no servers', async () => {
      ...
      await expect(client.callTool('test_tool', {})).rejects.toThrow('No MCP servers connected');
    });
  });
});
