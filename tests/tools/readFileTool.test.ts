...
import { readFileTool } from '../../src/plugins/tools/readFileTool';
...
describe('readFileTool', () => {
  ...
  it('should read specific lines from a file with explanation', async () => {
    ...
    const mockFileContent = Array.from({ length: 300 }, (_, i) => `Line ${i + 1}: Some code content here`).join('\n');
  ...
  it('should use default line range when line numbers not provided', async () => {
    const mockFileContent = Array.from({ length: 300 }, (_, i) => `Line ${i + 1}`).join('\n');
  ...
  it('should return entire file when file has fewer than 200 lines', async () => {
    const mockFileContent = Array.from({ length: 50 }, (_, i) => `Line ${i + 1}`).join('\n');
  ...
  it('should read specific line range from d2c store file', async () => {
    ...
    const mockFileLines = Array.from({ length: 1000 }, (_, i) => {
      ...
      if (lineNum === 876) return '  const processD2CConversion = async (data: any) => {';
      if (lineNum === 877) return '    // 核心转换逻辑在这里';
      if (lineNum === 878) return '    return await convertDataToComponent(data);';
    ...
    // 验证 fs 方法被正确调用
    expect(mockReadFile).toHaveBeenCalledWith('packages/ide-essential/src/stores/d2c/index.ts', 'utf-8');
    ...
    expect(result.content).toContain('const processD2CConversion = async (data: any) => {'); // 第876行
    expect(result.content).toContain('核心转换逻辑在这里'); // 第877行
    expect(result.content).toContain('return await convertDataToComponent(data);'); // 第878行
    ...
    const contentLines = lines.filter(
      (line) => !line.startsWith('[Lines') && !line.startsWith('[Adjusted range') && line.trim() !== '',
    );
  ...
  it('🔧 read_file ✅ Success Parameters: should read from line 200 to end of file when no end line specified', async () => {
    ...
    // 验证 fs 方法被正确调用
    expect(mockReadFile).toHaveBeenCalledWith('packages/code/tests/tools/grepSearchTool.test.ts', 'utf-8');
