...
function other() {
  console.log("different message");
    ...
    expect(result.content).toBe(
      "Successfully replaced 1 occurrence in test.js",
    );
  });
...
function test2() {
  ...
  it("should validate required parameters", async () => {
    ...
    expect(result.success).toBe(false);
    expect(result.error).toBe(
      "old_string parameter is required and must be a string",
    );
    ...
    expect(result.success).toBe(false);
    expect(result.error).toBe(
      "new_string parameter is required and must be a string",
    );

    // Test null new_string
    result = await searchReplaceTool.execute({
      file_path: "test.js",
      old_string: "test",
      new_string: null,
...
function test2() {
  ...
  it("should allow empty string as new_string (deletion)", async () => {
    const fileContent = `function test() {
  ...
  console.log("main message");
    ...
    const result: ToolResult = await searchReplaceTool.execute({
      file_path: "test.js",
      old_string: '  console.log("debug message");\n',
      new_string: "",
    });
