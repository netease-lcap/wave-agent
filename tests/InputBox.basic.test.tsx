...
import { InputBox, INPUT_PLACEHOLDER_TEXT } from '../src/components/InputBox';
...
describe('InputBox Basic Functionality', () => {
  ...
  it('should handle paste input with mixed content including @', async () => {
    ...
    // 模拟粘贴包含@符号和换行符的复杂文本
    const complexText = 'Please check @src/index.ts\nand also review\n@package.json file';
  ...
  it('should handle sequential paste operations correctly (React async state fix)', async () => {
    ...
    const expectedFullText = "const originalContent = await fs.promises.readFile(fullPath, 'utf-8');";
  ...
  it('should debounce paste operations and not show intermediate states', async () => {
    ...
    // 验证最终显示完整的合并内容
    const finalOutput = lastFrame();
    const expectedFullText = 'This is the first part of a very long text that gets pasted in multiple chunks';
    expect(finalOutput).toContain(expectedFullText);
  ...
  it('should log long text compression process', async () => {
    ...
    // 模拟粘贴超过200字符的长文本
    const longText = 'This is a very long text that exceeds 200 characters. ' + 'X'.repeat(160);
    stdin.write(longText);
